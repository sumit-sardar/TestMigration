...
public class StudentCompositeScoresController {
	  private StsTotalStudentScoreData totalData;
	    ...
	    private IrsLLRPCompositeFactMapper mapper;
	    ...
	    public StudentCompositeScoresController(Connection conn, StsTotalStudentScoreData totalData, CurriculumData currData, ContextData context) {
	        ...
	        mapper = new IrsLLRPCompositeFactMapper(conn);
	    }

	    public void run() throws SQLException {
	    	IrsLLRPCompositeFactData [] facts = getCompositeFactBeans();
	        for(int i=0;i<facts.length;i++) {
	        	IrsLLRPCompositeFactData newFact = facts[i];
	    ...
	    public IrsLLRPCompositeFactData [] getCompositeFactBeans() {
	        if(totalData.size() > 0) {
	            ...
	            for(int i=0;i<composites.length;i++) {
	               ...
	               if(total != null  && ("T".equals(total.getValidScore()) || "Y".equals(total.getValidScore())))  {
	            	   IrsLLRPCompositeFactData newFact = new IrsLLRPCompositeFactData();
	            ...
	            return (IrsLLRPCompositeFactData []) facts.toArray(new IrsLLRPCompositeFactData[0]);
	        } else {
	            return new IrsLLRPCompositeFactData[0];
	        }
	    }
}
