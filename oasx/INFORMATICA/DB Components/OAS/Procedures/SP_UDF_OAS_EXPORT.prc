CREATE OR REPLACE PROCEDURE SP_UDF_OAS_EXPORT
IS
...
-- FCAT Specific UDF position 60 has to be hard coded, since it is from   -- two sources.
 	...
 	--FCAT spring 2008 code change
      IF Customer_short_name='FCAT' AND TRIM (SUBSTR (v_udf, 187, 1)) IS NOT NULL
      THEN
         v_udf :=
               SUBSTR (v_udf, 1, 59)
            || SUBSTR (v_udf, 187, 1)
      ...
      END IF;
 	--FCAT spring 2008 code change
      IF Customer_short_name='FCAT' THEN  -- FCAT Specific UDF
      ...
      dbms_output.put_line('v_udf' || v_udf);
      -- UPDATE TEST_ROSTER --
      UPDATE TEST_ROSTER
         SET udf = v_udf
