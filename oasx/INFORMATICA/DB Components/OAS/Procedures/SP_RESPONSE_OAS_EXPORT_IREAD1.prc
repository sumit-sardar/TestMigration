CREATE OR REPLACE PROCEDURE SP_RESPONSE_OAS_EXPORT_IREAD1 IS
  ...
  CURSOR CUR_EXP_CR_DECISION IS
    SELECT VALUE
      ...
      FROM OAS_STAGE_IRD.STG_EXPORT_PARMS PARAM, STG_PARAM_WINSCR_TB STG
     WHERE UPPER(PARAM.PARAMETER) = 'CR_DECISION'
       AND PARAM.CUSTOMER_ID = STG.CUSTOMER_ID;

  --cursor for test section name *** added for IREAD Spring09
  CURSOR CUR_TEST_SECTION_NAME(P_CUSTOMER_ID INTEGER, P_OAS_TS_NAME VARCHAR2) IS
    SELECT VALUE
      FROM OAS_STAGE_IRD.STG_EXPORT_PARMS STG
...
BEGIN
        ...
        FOR QUESTION_NO IN 1 .. V_SR_RESP_TAB.COUNT LOOP
         ...
         -- To check the time taken by system
          ...
          ELSE
            ...
            dbms_output.put_line('TSTSTSTS'||C_ST_REC.CUSTOMER_ID);
  ...
  END IF;
END SP_RESPONSE_OAS_EXPORT_IREAD1;
/
