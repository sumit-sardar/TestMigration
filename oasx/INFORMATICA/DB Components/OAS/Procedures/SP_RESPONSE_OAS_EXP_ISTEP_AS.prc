CREATE OR REPLACE PROCEDURE SP_RESPONSE_OAS_EXP_ISTEP_AS IS
...
BEGIN
    ...
    FOR C_ST_REC IN C_ST LOOP
      ...
      LOOP
        ...
        FOR QUESTION_NO IN 1 .. V_SR_RESP_TAB.COUNT LOOP
          ...
          OPEN CUR_TEST_SECTION_NAME(C_ST_REC.CUSTOMER_ID,
                                     V_SR_RESP_TAB(QUESTION_NO).TD_SUBJECT);
          FETCH CUR_TEST_SECTION_NAME
            INTO V_SUBJECT;
          ...
          ELSE
            ...
            V_ERASURE_COUNT := ERASURE_COUNT(C_ST_REC.TEST_ROSTER_ID,
                                             V_SR_RESP_TAB(QUESTION_NO)
                                             .TD_ITEM_SET_ID);
            IF (V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID = V_SR_RESP_TAB(1)
               .TD_ITEM_SET_ID) THEN
              /* This check is to comapre subtest_id of the last record with the same of first record.If same then
              the variable v_ea,v_ra,v_sa,v_sum will be Added with value of
              the last record*/
              V_GLOBAL_TS_ID := V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID;
            END IF;
  ...
  END IF;
END SP_RESPONSE_OAS_EXP_ISTEP_AS;
/
