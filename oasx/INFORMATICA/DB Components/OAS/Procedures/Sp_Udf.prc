CREATE OR REPLACE PROCEDURE Sp_Udf
IS
...
/*
      SELECT DISTINCT tr.student_id, tr.test_roster_id, tr.org_node_id,
                      tr.test_admin_id, tr.validation_status, ta.LOCATION,
                      TRIM (UPPER (tc.subject)) subject, tr.form_assignment
                 FROM STUDENT st,
                      TEST_ROSTER tr,
                ...
                WHERE st.student_id = tr.student_id
                  ...
                  AND ta.test_admin_id IN (30167, 30169)
                  AND EXISTS (
                         SELECT NULL
                           FROM stg_eiss_tb se
                          WHERE se.level_type = 'SR'
                            AND se.sr_id = st.student_id
                            AND se.gh_id = tr.org_node_id)
      UNION
      SELECT DISTINCT tr.student_id, tr.test_roster_id, tr.org_node_id,
                      tr.test_admin_id, tr.validation_status, ta.LOCATION,
                      TRIM (UPPER (tc.subject)) subject, tr.form_assignment
                ...
                WHERE st.student_id = tr.student_id
                  ...
                  AND tr.activation_status = 'AC'
                  AND tr.validation_status IN ('VA', 'IN')
                  AND tr.test_completion_status IN ('NT', 'SC')
                  AND ta.test_admin_id IN (30167, 30169)
                  AND st.student_id = srd.student_id(+)
                  AND srd.data_key LIKE
                                'Absent%' || TRIM (REPLACE (tc.subject, ' I'))
                  AND srd.data_value = 'Yes'
                  AND EXISTS (
                         SELECT NULL
                           FROM stg_eiss_tb se
                          WHERE se.level_type = 'SR'
...
*/
   ...
   /*
         SELECT DISTINCT tr.student_id, tr.test_roster_id, tr.org_node_id,
                         tr.test_admin_id, tr.validation_status, ta.LOCATION,
                         UPPER (tc.subject) subject
                    -- validation_status, location and subject (provided by Sudha) for FCAT -
         FROM            STUDENT st,
                         TEST_ROSTER tr,
                         TEST_ADMIN ta,
                         TEST_CATALOG tc,
         ...
         BULK COLLECT INTO v_arr_srd_udf_val
             FROM (SELECT srd.data_value, oas_column_name,
                    ...
                    WHERE srd.student_id(+) = cur_tr_rec.student_id
                      ...
                      AND oas_column_name = crn.data_name) a,
...
-- Keep if for FCAT --
         ...
         THEN
            -- Reading Vs Matths --
            IF UPPER (v_udf_table (v_udf_rec_no).oas_table_name) LIKE
                                             '%' || cur_tr_rec.subject || '%'
            THEN
               v_udf_curr_val :=
...
-- Keep if for FCAT --
         ...
         ELSIF     UPPER (v_udf_table (v_udf_rec_no).oas_table_name) =
               ...
               AND UPPER (v_udf_table (v_udf_rec_no).oas_column_name) =
                                                                    'LOCATION'
         THEN
            v_udf_curr_val := cur_tr_rec.LOCATION;
...
--
         ELSIF     UPPER (v_udf_table (v_udf_rec_no).oas_table_name) LIKE
               ...
               AND UPPER (v_udf_table (v_udf_rec_no).oas_column_name) =
                                                             'FORM_ASSIGNMENT'
         THEN
            v_udf_curr_val := cur_tr_rec.form_assignment;
         END IF;
