CREATE OR REPLACE PROCEDURE SP_UDF_OAS_EXPORT_IREAD IS
  ...
  FOR CUR_TR_REC IN CUR_TR LOOP
    ...
    FOR V_UDF_REC_NO IN 1 .. V_UDF_LAST_REC_NO LOOP
      ...
      ELSIF UPPER(V_UDF_TABLE(V_UDF_REC_NO).OAS_TABLE_NAME) =
        ...
        IF CUSTOMER_SHORT_NAME = 'IREAD' THEN
          V_UDF_CURR_VAL := FN_UDF_VALIDATION_EXPORT_IREAD(CUR_TR_REC.STUDENT_ID,
          
                                                           CUR_TR_REC.TEST_ROSTER_ID);
          --Modified   for IREAD  Spring 10                                               
        ...
        -- Added for IREAD Spring 2010 to have the contents for position 94 to 100
        ELSIF UPPER(V_UDF_TABLE(V_UDF_REC_NO).OAS_TABLE_NAME) =
    ...
    --Added  for IREAD. As for IREAD the udf field length is 147. from 91 to 147 we have to hard code with space
    IF CUSTOMER_SHORT_NAME = 'IREAD' THEN
