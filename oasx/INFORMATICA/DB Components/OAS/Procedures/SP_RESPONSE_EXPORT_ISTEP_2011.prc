CREATE OR REPLACE PROCEDURE SP_RESPONSE_EXPORT_ISTEP_2011 IS
  ...
  --Extract all the Item_Set_Ids, Item_Ids along with their Correct answers and Order them as to how they were ordered at the test
    SELECT TR.TEST_ROSTER_ID AS TEST_ROSTER_ID ,
     ...
     WHERE TA.PRODUCT_ID = TC.PRODUCT_ID
       ...
       CURSOR C_TD_TEST (IN_ITEM_ID NUMBER,IN_ITEM_SET_ID NUMBER,IN_TEST_ROSTER_ID NUMBER,IN_TD_ITEM_SET_SORT_ORDER NUMBER,SI_ITEM_SET_ORDER NUMBER,IN_CORRECT_ANSWER VARCHAR2,ITEM_SORT_ORDER NUMBER,IN_TD_SUBJECT VARCHAR2) is
       
      SELECT IN_TD_SUBJECT,
       ...
       DECODE(IN_CORRECT_ANSWER,
              ...
              '*') AS CORRECT_ANSWER,
       (CASE
         WHEN (RESPONSE = IN_CORRECT_ANSWER) THEN
          '1'
         ELSE
          '0'
       END) AS SCORE,
       ...
       WHERE IN_TEST_ROSTER_ID = IR.TEST_ROSTER_ID
            --Outer Joins performed to account for all Items, Item sets that were unanswered or omitted --
         AND IN_ITEM_SET_ID = IR.ITEM_SET_ID
         AND IN_ITEM_ID = IR.ITEM_ID
         AND (IR.RESPONSE_SEQ_NUM IS NULL OR
             IR.RESPONSE_SEQ_NUM =
             (SELECT MAX(IRM.RESPONSE_SEQ_NUM)
                 FROM ITEM_RESPONSE IRM
                WHERE IN_TEST_ROSTER_ID = IRM.TEST_ROSTER_ID
                  AND IN_ITEM_SET_ID = IRM.ITEM_SET_ID
                  AND IN_ITEM_ID = IRM.ITEM_ID
                GROUP BY IN_TEST_ROSTER_ID, IN_ITEM_SET_ID, IN_ITEM_ID))
       ORDER BY SI_ITEM_SET_ORDER,
          IN_TD_ITEM_SET_SORT_ORDER,
          QUES_NO,
         IN_ITEM_SET_ID; /*Get the records order by subtest id*/
  ...
  CURSOR C_GI(IN_TEST_ROSTER_ID NUMBER, IN_STUDENT NUMBER, IN_TC_ID NUMBER) IS WITH MAIN AS(
    SELECT TR.TEST_ROSTER_ID AS TEST_ROSTER_ID,
     ...
     WHERE 1 = 1
       ...
       AND TA.CUSTOMER_ID = V_CUSTOMER_ID
       AND TA.PRODUCT_ID = V_PRODUCT_ID
...
BEGIN
  --Select ST records from STG_EISS_Tb table
   ...
   BEGIN 
        SELECT CUSTOMER_ID  , PRODUCT_ID INTO 
         V_CUSTOMER_ID,V_PRODUCT_ID 
         FROM STG_PARAM_WINSCR_TB WHERE SNO=1;
      EXCEPTION
            WHEN NO_DATA_FOUND THEN
              RAISE_APPLICATION_ERROR(-20000,
                                'Customer id ,product ID
                                 is not provided in params table.');
   END;
  ...
  END IF;
END SP_RESPONSE_EXPORT_ISTEP_2011;
/
