  ...
  CURSOR C_TD(IN_TEST_ROSTER_ID NUMBER) IS WITH MAIN AS(
    SELECT IR.TEST_ROSTER_ID AS TEST_ROSTER_ID,
     ...
     WHERE TEST_ROSTER_ID = IN_TEST_ROSTER_ID
       ...
       AND ITEM.ANSWER_AREA is null  ----change for laslink
    ...
    SELECT M.TEST_ROSTER_ID,
     ...
     and IR.TEST_ROSTER_ID = M.TEST_ROSTER_ID
     and IR.ITEM_SET_ID = M.TD_ITEM_SET_ID;
  ...
  FOR C_ST_REC IN C_ST LOOP
    ...
    LOOP
      ...
      FOR QUESTION_NO IN 1 .. V_SR_RESP_TAB.COUNT LOOP
        ...
        RESP := FN_GETCRRESPONSE(V_SR_RESP_TAB(QUESTION_NO).TEST_ROSTER_ID,
                                 V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID,
                                 V_SR_RESP_TAB(QUESTION_NO).ITEM_ID);
        IF (RESP = 1) THEN
           SELECT RESP INTO V_RA FROM TEST_RESP;
        ELSE 
            SELECT  ' ' into V_RA from dual;
        end if;
    ...
    CLOSE C_TD;
  END LOOP;
END SP_CRRESPONSE_OAS_EXPORT_Lslnk;
/
