create or replace procedure sp_opunit_roster_calibration is
...
v_calibration_flag  varchar2(1);

--cursor for extracting test_roster_id from stg_test_roster table
cursor cur_testroster is 
 select test_roster_id,opunit,calibration from stg_test_roster where opunit is null;
...
cursor_last_opunit_rec cur_exp_last_opunit%rowtype;
  --*************************************************************************************************************************************--
                          /**Modification for calibration*/
                          --cursor for stg_export_parms table
                          cursor cur_exp_calibration
                          is
                          select value from stg_export_parms where upper(parameter)='CALIBRATION_FLAG' and customer_id = v_cust_id ; 
                          --cursor rowtype declaration
                          cursor_calibration cur_exp_calibration%rowtype;
                          
                          --cursor for stg_export_parms table
                          cursor cur_exp_opunit_calib
                          is
                          select value from stg_export_parms where upper(parameter)='START_OPUNIT_CALIB' and customer_id = v_cust_id ; 
                          
                          --cursor rowtype declaration
                          cursor_opunit_calib_rec cur_exp_opunit_calib%rowtype;
                          
                          
                          --cursor for stg_export_parms table TO get opunit range
                          cursor cur_last_opunit_calib
                          is
                          select value from stg_export_parms where upper(parameter)='LAST_OPUNIT_CALIB' and customer_id = v_cust_id ; 
                          
                          --cursor rowtype declaration
                          cursor_last_opunit_calib_rec cur_last_opunit_calib%rowtype;
                          /** END Modification for calibration*/
    --*************************************************************************************************************************************--              
 ...
 close cur_exp_last_opunit;
  ...
  --*************************************************************************************************************************************--
            ...
            open cur_exp_calibration;
               fetch cur_exp_calibration into cursor_calibration;
             
             if cur_exp_calibration%notfound then
                v_calibration_flag  := 'N';
             else 
                 v_calibration_flag  := 'Y';
             end if;
             close cur_exp_calibration;
             
             
             if ( v_calibration_flag  = 'Y' ) then
                
                open cur_exp_opunit_calib; 
                  fetch cur_exp_opunit_calib  into cursor_opunit_calib_rec;
                  
                if cur_exp_opunit_calib%notfound then
                    raise_application_error (-20001,'last_opunit value not set for calibration' || v_cust_id);
                end if;
                close cur_exp_opunit_calib; 
                
                
                
                 open cur_last_opunit_calib; 
                  fetch cur_last_opunit_calib into cursor_last_opunit_calib_rec ;
                  
                if cur_last_opunit_calib%notfound then
                    raise_application_error (-20001,'last_opunit value not set for calibration' || v_cust_id);
                end if;
                close cur_last_opunit_calib; 
            end if;
 ...
 --      dbms_output.put_line ('inside cur_dist_opunit%notfound loop');
     ...
     ---calibration opunit range
      if ( v_calibration_flag  = 'Y' ) then
       ...
       update stg_test_roster set opunit= cursor_opunit_calib_rec.value
       where 
       test_roster_id = cur_testroster1.test_roster_id;
       
       update test_roster set opunit = cursor_opunit_calib_rec.value 
       where 
       test_roster_id = cur_testroster1.test_roster_id;
     end if;
     
      ---Non calibration opunit range
     if ( v_calibration_flag  = 'N' ) then
   ...
   --                      dbms_output.put_line ('else part of <2999- will insert recs into all 3 tables');         
                         ...
                         cursor_opunit_calib_rec.value:= cursor_dop_rec.opunit  +1; 
