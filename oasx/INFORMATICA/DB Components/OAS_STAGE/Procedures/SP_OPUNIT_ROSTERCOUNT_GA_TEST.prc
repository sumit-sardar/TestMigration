CREATE OR REPLACE PROCEDURE SP_OPUNIT_ROSTERCOUNT_GA_TEST IS
  ...
  BEGIN
    ...
    FOR CUR_TESTROSTER1 IN CUR_TESTROSTER LOOP
      ...
      --dbms_output.put_line(CURSOR_DIST_ORGID_REC.ORG_NODE_ID);
      ...
      ELSE
        ...
        /*When the procedure starts execution 1st time V_REEXPORT will be false. If it is false
         then new opunit will assigned it to by adding 1 to CURSOR_DOP_REC.OPUNIT  and new record
         will be inserted into STG_DISTRICT_OPUNIT and while processing other test_roster records 
         of this execution process the v_export value will be true and they will be assigned with
         this new opunit and  STG_DISTRICT_OPUNIT record will be updated with the roster count.(according to 
         roster count limitation condition )*/
      
        if (V_REEXPORT = false) then
          ...
          IF (CURSOR_DOP_REC.ROSTER_COUNT < CUR_EXP_ROSTER_COUNT_REC.VALUE) THEN
            --this part is executed if roster_count < limited size of roster count
          ...
          ELSE
            ...
            IF (FLAG) THEN
              dbms_output.put_line(CUR_TESTROSTER1.TEST_ROSTER_ID);
              UPDATE STG_TEST_ROSTER
                 SET OPUNIT = V_OPUNIT
               WHERE TEST_ROSTER_ID = CUR_TESTROSTER1.TEST_ROSTER_ID;
            ...
            IF (V_OPUNIT <= CURSOR_LAST_OPUNIT_REC.VALUE) THEN
              ...
              -- Check the occurence of same class organization 
              --If flag value = false then no same class org.
              IF (FLAG = FALSE) THEN
