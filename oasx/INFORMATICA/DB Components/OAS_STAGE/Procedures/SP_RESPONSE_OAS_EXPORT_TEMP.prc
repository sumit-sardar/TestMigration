CREATE OR REPLACE PROCEDURE SP_RESPONSE_OAS_EXPORT_TEMP IS
  ...
  /* temp variable declarations ENDS here ------*/
  
  V_INDEX_EISS_TB Integer :=0;
  ...
  V_SR_RESP_TAB SR_RESP_TAB;
   ...
   TYPE eiss_tb_REC IS RECORD(
    CUSTOMER_ID    NUMBER(10),
  PRODUCT_ID     NUMBER(10),
  TEST_ROSTER_ID NUMBER(10),
  OU_ID          NUMBER(10),
  SH_ID          NUMBER(10),
  GH_ID          NUMBER(10),
  SR_ID          NUMBER(10),
  ...
  LEVEL_DATA     VARCHAR2(2000),
  OPUNIT         VARCHAR2(3));

  TYPE insert_eiss_TAB IS TABLE OF eiss_tb_REC INDEX BY BINARY_INTEGER;

  V_insert_eiss_TAB insert_eiss_TAB;
  ...
  CURSOR C_GI(IN_TEST_ROSTER_ID NUMBER, IN_STUDENT NUMBER, IN_TC_ID NUMBER) IS WITH MAIN AS(
    SELECT TR.TEST_ROSTER_ID AS TEST_ROSTER_ID,
     ...
     WHERE 1 = 1
      AND 
      --I.ITEM_ID = GDE.ITEM_ID --FCAT Sprin 2008
        --TR.FORM_ASSIGNMENT = GDE.FORM_ASSIGNMENT --FCAT Sprin 2008
       --AND 
       TA.PRODUCT_ID = TC.PRODUCT_ID
       AND TA.TEST_CATALOG_ID = TC.TEST_CATALOG_ID
       AND TR.TEST_ADMIN_ID = TA.TEST_ADMIN_ID
       AND TR.CUSTOMER_ID = TA.CUSTOMER_ID
       AND ISI.ITEM_SET_ID = ISTD.ITEM_SET_ID
...
BEGIN
  ...
  nTotalTimeTaken := dbms_output.put_line('CUR_EXP_CR_DECISION time taken ' || (nEndTime  - nStartTime));
        ...
        FOR QUESTION_NO IN 1 .. V_SR_RESP_TAB.COUNT LOOP
          ...
          IF (QUESTION_NO < V_SR_RESP_TAB.COUNT) THEN
            ...
            IF (V_SR_RESP_TAB(QUESTION_NO)
              ...
              insert_eiss_TAB(V_INDEX_EISS_TB).CUSTOMER_ID :=C_ST_REC.CUSTOMER_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).PRODUCT_ID :=C_ST_REC.PRODUCT_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).TEST_ROSTER_ID :=C_ST_REC.TEST_ROSTER_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).OU_ID :=C_ST_REC.OU_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).SH_ID :=C_ST_REC.SH_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).GH_ID :=C_ST_REC.GH_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).SR_ID :=C_ST_REC.SR_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).ST_ID :=C_ST_REC.ST_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).TS_ID :=V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID; /*insert the subtest_id*/
              insert_eiss_TAB(V_INDEX_EISS_TB).GRADE :=C_ST_REC.GRADE;
              insert_eiss_TAB(V_INDEX_EISS_TB).SORT_ORDER :=V_SR_RESP_TAB(QUESTION_NO).TS_ORDER;
             
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_TYPE := 'TS';
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_DATA :='TS ' || V_SUBJECT || ',' || V_SECTTYP || ',' ||
              ...
              V_INDEX_EISS_TB := V_INDEX_EISS_TB+1;
              insert_eiss_TAB(V_INDEX_EISS_TB).CUSTOMER_ID :=C_ST_REC.CUSTOMER_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).PRODUCT_ID :=C_ST_REC.PRODUCT_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).TEST_ROSTER_ID :=C_ST_REC.TEST_ROSTER_ID;
              insert_eiss_TAB(V_INDEX_EISS_TB).OU_ID :=C_ST_REC.OU_ID;
              ...
              insert_eiss_TAB(V_INDEX_EISS_TB).TS_ID :=V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID; /*insert the subtest_id*/
              insert_eiss_TAB(V_INDEX_EISS_TB).GRADE :=C_ST_REC.GRADE;
              insert_eiss_TAB(V_INDEX_EISS_TB).SORT_ORDER :='4';
             
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_TYPE := 'TD';
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_DATA :='RA ' || V_RA;
              ...
              insert_eiss_TAB(V_INDEX_EISS_TB).TS_ID :=V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID; /*insert the subtest_id*/
              insert_eiss_TAB(V_INDEX_EISS_TB).GRADE :=C_ST_REC.GRADE;
              insert_eiss_TAB(V_INDEX_EISS_TB).SORT_ORDER :='5';
             
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_TYPE := 'TD';
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_DATA :='SA ' || V_SA;
              ...
              insert_eiss_TAB(V_INDEX_EISS_TB).TS_ID :=V_SR_RESP_TAB(QUESTION_NO).TD_ITEM_SET_ID; /*insert the subtest_id*/
              insert_eiss_TAB(V_INDEX_EISS_TB).GRADE :=C_ST_REC.GRADE;
              insert_eiss_TAB(V_INDEX_EISS_TB).SORT_ORDER :='6';
             
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_TYPE := 'TD';
              insert_eiss_TAB(V_INDEX_EISS_TB).LEVEL_DATA :='EA ' || V_EA;
          ...
          FORALL i IN 1..insert_eiss_TAB.COUNT
                 INSERT INTO STG_EISS_TB VALUES insert_eiss_TAB(i);
  ...
  END IF;
END SP_RESPONSE_OAS_EXPORT_TEMP;
/
