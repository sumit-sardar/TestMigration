CREATE OR REPLACE FUNCTION FUN_MONARCH_STUDENT_COUNT_GA(IN_ORG_NODE_ID IN TEST_ROSTER.ORG_NODE_ID%TYPE,
  ...
  CURSOR SGL_COUNT IS
    SELECT COUNT(TR.STUDENT_ID)
     ...
     WHERE TR.ORG_NODE_ID = IN_ORG_NODE_ID
       ...
       AND TC.PRODUCT_ID = IN_PRODUCT_ID;
...
BEGIN
  ...
  */
  ...
  RETURN V_COUNT;
END FUN_MONARCH_STUDENT_COUNT_GA;
/
