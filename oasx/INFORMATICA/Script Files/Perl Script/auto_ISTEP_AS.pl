...
$workflow_par_file='/appl/informatica/PowerCenter8.1.1/server/infa_shared/SrcFiles/HWDI/parameter/oas_eiss.par';
...
open my $in,"<","$workflow_par_file" or die "Can't read old file: $!"; 
open my $out,">","$workflow_par_file.new" or die "Can't write new file: $!"; 

my $temp = '$$ORG_NODE_ID=';
my $line = '[OAS_Export.s_m_O_TEST_ROSTER_ID_Extract_ISTEP]';
while( <$in> ) { # print the lines before the change 
    print $out $_; 
    last if index($_,$line) > -1; 
} 

print $out "$district_id"; 

while( <$in> ) { # print the rest of the lines 
    next if index($_,$temp) > -1; 
    print $out $_; 
}

rename("$workflow_par_file.new","$workflow_par_file") or die("Can't rename: $!\n");  
system("echo \" \n\n ******* Calling Workflows District wise******** \n \" >> $log_file ");


system("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd startworkflow -sv $integration_service -d $domain -u $user_name -p $passwd -wait wf_eiss_ISTEP_Applied_Skills >> $log_file");
system ("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd  getworkflowdetails -sv $integration_service -d $domain -u $user_name -p $passwd wf_eiss_ISTEP_Applied_Skills >>  $log_file");

system("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd startworkflow -sv $integration_service -d $domain -u $user_name -p $passwd -wait wf_MONARCH_Completeness_Check >> $log_file");
system ("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd  getworkflowdetails -sv $integration_service -d $domain -u $user_name -p $passwd wf_MONARCH_Completeness_Check >>  $log_file");
