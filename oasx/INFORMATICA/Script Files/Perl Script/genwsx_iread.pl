...
$passwd = 'ayan321';
$target_dir = '/appl/informatica/PowerCenter8.1.1/server/infa_shared/TgtFiles/HWDI/Track_A2/EISS/IREAD';
...
#        system ("pmcmd  getworkflowdetails -s $server:4001 -u $user_name -p $passwd wf_eiss_level_dataload >>  $log_file");
        system("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd startworkflow -sv $integration_service -d $domain -u $user_name -p $passwd -wait wf_eiss_level_dataload_IREAD >> $log_file");
        system ("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd  getworkflowdetails -sv $integration_service -d $domain -u $user_name -p $passwd wf_eiss_level_dataload_IREAD >>  $log_file");
         

        system("sh /appl/informatica/PowerCenter8.1.1/server/infa_shared/Scripts/HWDI/file_split_iread.sh $target_dir $archive_dir $par_dir $opunit");
