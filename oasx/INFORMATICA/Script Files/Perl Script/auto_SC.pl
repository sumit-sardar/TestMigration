...
system("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd startworkflow -sv $integration_service -d $domain -u $user_name -p $passwd -wait wf_eiss_ISTEP_SC >> $log_file");
system ("/appl/informatica/PowerCenter8.1.1/server/bin/pmcmd  getworkflowdetails -sv $integration_service -d $domain -u $user_name -p $passwd wf_eiss_ISTEP_SC >>  $log_file");
