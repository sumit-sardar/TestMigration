...
CREATE OR REPLACE PACKAGE BODY OAS_OK_TEST_PRE_SCHEDULING IS
  ...
  NOT_TESTED_CODE_DEMO_NAME       CONSTANT CUSTOMER_DEMOGRAPHIC.LABEL_NAME%TYPE:= 'Not Tested Codes';
  ...
  CURSOR CUR_GET_TCTEST_FOR_PRODUCT(V_PRODUCTID INTEGER, V_USERID USERS.USER_ID%TYPE) IS
   ...
   CURSOR CUR_STU_FOR_WINTER_TRIMESTER(V_DEPTT_NODE_ID        ORG_NODE_ANCESTOR.ORG_NODE_ID%TYPE, 
                                       ...
                                       V_TEST_NAME            TEST_CATALOG.TEST_NAME%TYPE,
                                       V_NOT_TESTED_DEMO_ID   CUSTOMER_DEMOGRAPHIC.CUSTOMER_DEMOGRAPHIC_ID%TYPE) IS
    SELECT STU.STUDENT_ID,
           STU.USER_NAME,
           STU.ACTIVATION_STATUS,
           STU.GRADE,
    ...
    SELECT STU.STUDENT_ID,
     ...
     (SELECT *
              FROM STUDENT_DEMOGRAPHIC_DATA DEMO
             WHERE DEMO.STUDENT_ID = STU.STUDENT_ID
               AND DEMO.CUSTOMER_DEMOGRAPHIC_ID = V_NOT_TESTED_DEMO_ID)
    /*AND EXISTS (SELECT *
                              FROM STUDENT_DEMOGRAPHIC_DATA DEMO
                             WHERE DEMO.STUDENT_ID = STU.STUDENT_ID
                               AND DEMO.VALUE_NAME = V_TESTING_SUBJECT)*/
  ...
  PROCEDURE INSERT_TEST_ROSTER_WINTER(VPRODUCTID              INTEGER,
                                      ...
                                      V_TEST_WINDOW           STUDENT_DEMOGRAPHIC_DATA.VALUE_NAME%TYPE,
                                      V_TEST_NAME             TEST_CATALOG.TEST_NAME%TYPE,
                                      V_NOT_TESTED_DEMO_ID    CUSTOMER_DEMOGRAPHIC.CUSTOMER_DEMOGRAPHIC_ID%TYPE) AS

    V_IS_CURSOR_RETURNS_VALUE BOOLEAN := FALSE;

  BEGIN

    OPEN CUR_STU_FOR_WINTER_TRIMESTER(ORG_NODE_ID, VPRODUCTID,V_TESTING_SUBJECT,V_TEST_WINDOW,V_TEST_NAME,V_NOT_TESTED_DEMO_ID);
    LOOP
  ...
  PROCEDURE SCHEDULE_SESSIONS_FOR_OK(INPUT_CUSTOMER_ID     INTEGER,
    ...
    V_TESTING_SUBJECT            STUDENT_DEMOGRAPHIC_DATA.VALUE_NAME%TYPE;
    V_TEST_WINDOW                STUDENT_DEMOGRAPHIC_DATA.VALUE_NAME%TYPE;
    V_NOT_TESTED_DEMO_ID         CUSTOMER_DEMOGRAPHIC.CUSTOMER_DEMOGRAPHIC_ID%TYPE;
                ...
                /* DBMS_OUTPUT.PUT_LINE(V_SUBJECT_NAME);*/
                 ...
                 IF INSTR(V_TEST_NAME, V_SUBJECT_NAME) != 0 THEN
                    ...
                    /*DBMS_OUTPUT.PUT_LINE(V_TEST_NAME);*/
                      ...
                      BEGIN
                        SELECT CD.CUSTOMER_DEMOGRAPHIC_ID
                          INTO V_NOT_TESTED_DEMO_ID
                          FROM CUSTOMER_DEMOGRAPHIC CD
                         WHERE CD.CUSTOMER_ID = INPUT_CUSTOMER_ID
                           AND CD.LABEL_NAME = NOT_TESTED_CODE_DEMO_NAME;
                      EXCEPTION
                        WHEN NO_DATA_FOUND THEN
                          LOG_MESSAGE(V_LOG_ID,
                                      V_STATUS_ERROR,
                                      'No record found in customer_demographic for label name ['|| NOT_TESTED_CODE_DEMO_NAME ||'].');
                          V_NOT_TESTED_DEMO_ID := null;
                      END;
