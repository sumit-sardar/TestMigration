CREATE OR REPLACE PACKAGE OAS_OK_3_8_TEST_PRE_SCHEDULING IS
...
*/
  ...
  PROCEDURE SCHEDULE_SESSIONS_FOR_OK_3_8(INPUT_CUSTOMER_ID     INTEGER,
                                         INPUT_TESTING_WINDOW  VARCHAR2,
                                         SCHEDULING_TYPE       VARCHAR2);
                               
                                     
END OAS_OK_3_8_TEST_PRE_SCHEDULING;
/
CREATE OR REPLACE PACKAGE BODY OAS_OK_3_8_TEST_PRE_SCHEDULING IS

  /* Constant declarations */

  SPRING_FORM_3_8_PRODUCT_ID      CONSTANT PRODUCT.PRODUCT_ID%TYPE := 9008;  
  ...
  FORM_ASSIGNMENT_METHOD          VARCHAR2(10);
  ...
  V_TEST_ADMIN_NAME               TEST_ADMIN.TEST_ADMIN_NAME%TYPE;
  V_FORM_ASSIGNMENT               TEST_ROSTER.FORM_ASSIGNMENT%TYPE;
  PREFERRED_FORM                  TEST_ADMIN.PREFERRED_FORM%TYPE := 1;
  VROSTER_STATUS_FLAG             VARCHAR2(300) := 'Roster_Status_Flag';
  V_ARRAY_FORM                    ARRAY_OF_STR;
  V_ARRAY_GRADE                   ARRAY_OF_STR;
  V_ARRAY_CONTENT_AREA            STRING_ARRAY;
  ...
  CURSOR CUR_GET_FORM_ASSIGNMENT(VTESTADMINID TEST_ADMIN.TEST_ADMIN_ID%TYPE) IS
  ...
  CURSOR CUR_GET_TCTEST_FOR_PRODUCT(V_PRODUCTID INTEGER, V_USERID USERS.USER_ID%TYPE) IS
   ...
   CURSOR CUR_GET_STU_FOR_OK_3_8(V_DEPTT_NODE_ID   ORG_NODE_ANCESTOR.ORG_NODE_ID%TYPE, 
                                 V_PRODUCT_ID      INTEGER,
                                 V_TC_ITEM_SET_ID  TEST_CATALOG.ITEM_SET_ID%TYPE,
                                 V_STU_GRADE       STUDENT.GRADE%TYPE) IS
     ...
     WHERE ONS.STUDENT_ID = STU.STUDENT_ID
       ...
       AND (SELECT DECODE(COUNT(1), 0, 1, 0)
             ...
             WHERE ROS.STUDENT_ID = STU.STUDENT_ID
               ...
               AND ADM.ITEM_SET_ID = V_TC_ITEM_SET_ID -- Checking whether students is already associated with test session for the content area,if the student is present in multiple deptt(content areas)
  ...
  /*CURSOR CUR_GET_STU_FOR_OTHER_GRADES (V_DEPTT_NODE_ID   ORG_NODE_ANCESTOR.ORG_NODE_ID%TYPE, 
                                       V_PRODUCT_ID      INTEGER,
                                       V_TC_ITEM_SET_ID  TEST_CATALOG.ITEM_SET_ID%TYPE,
                                       V_STU_GRADE       STUDENT.GRADE%TYPE) IS
    SELECT STU.STUDENT_ID,
  ...
  CURSOR CUR_GET_STU_FOR_OTHER_GRADES (V_DEPTT_NODE_ID   ORG_NODE_ANCESTOR.ORG_NODE_ID%TYPE, 
                                       ...
                                       V_TC_ITEM_SET_ID  TEST_CATALOG.ITEM_SET_ID%TYPE,
                                       V_STU_GRADE       STUDENT.GRADE%TYPE) IS
   SELECT PARENTNODE.PARENT_ORG_NODE_ID,
          RTRIM(XMLAGG(XMLELEMENT(E,
                                  '{' || STU.STUDENT_ID || '$' ||
                                  STU.USER_NAME || '$' || STU.GRADE || '$' ||
                                  NODE.ORG_NODE_NAME || '}, '))
                .EXTRACT('//text()'),
                ', ') LOGGIN_INFO
     FROM ORG_NODE_STUDENT ONS,
          ...
          ORG_NODE NODE,
          STUDENT_ACCOMMODATION ACCO,
          (SELECT ORG_NODE_ID, PARENT_ORG_NODE_ID
             FROM ORG_NODE_PARENT
            WHERE PARENT_ORG_NODE_ID = V_DEPTT_NODE_ID) PARENTNODE
    WHERE ONS.STUDENT_ID = STU.STUDENT_ID
      AND STU.STUDENT_ID = ACCO.STUDENT_ID
      AND ONS.ACTIVATION_STATUS = V_ACTIVATION_STATUS
      AND STU.ACTIVATION_STATUS = V_ACTIVATION_STATUS
      ...
      AND STU.GRADE <> V_STU_GRADE
      AND ONS.ORG_NODE_ID = NODE.ORG_NODE_ID
      AND NODE.ORG_NODE_ID = PARENTNODE.ORG_NODE_ID
      AND ONS.ORG_NODE_ID IN
          (SELECT ORG_NODE_ID
      ...
      /*AND (SELECT DECODE(COUNT(1), 0, 1, 0)
            ...
            WHERE ROS.STUDENT_ID = STU.STUDENT_ID
              ...
              AND ADM.ITEM_SET_ID = V_TC_ITEM_SET_ID -- Checking whether students is already associated with test session for the content area,if the student is present in multiple deptt(content areas)
              AND ADM.PRODUCT_ID = V_PRODUCT_ID) = 1*/
    GROUP BY PARENTNODE.PARENT_ORG_NODE_ID;
  ...
  TYPE ARR_STU_FOR_TESTING_WINDOW IS TABLE OF CUR_GET_STU_FOR_OK_3_8%ROWTYPE;
  V_STUDENT          CUR_GET_STU_FOR_OK_3_8%ROWTYPE;
  V_STU_OTHER_GRADES CUR_GET_STU_FOR_OTHER_GRADES%ROWTYPE;
  ...
  PROCEDURE INITIALIZE_GLOBAL_ARRAYS AS
  BEGIN
    V_ARRAY_GRADE(1) := '06';
    V_ARRAY_GRADE(2) := '07';
    V_ARRAY_GRADE(3) := '08';
    V_ARRAY_CONTENT_AREA(1) := 'READING';
    V_ARRAY_CONTENT_AREA(2) := 'MATHEMATICS';
    V_ARRAY_CONTENT_AREA(3) := 'GEOGRAPHY';
  END;
  ...
  BEGIN
    ...
    V_TEST_ADMIN_NAME := RECTCITEMSETID.ITEM_SET_DISPLAY_NAME|| ' - ' || V_SESSION_SEQUENCE;
  ...
  PROCEDURE INSERT_TEST_ROSTER_FIXED(VPRODUCTID                  INTEGER,
                                     VCUSTOMERID                 INTEGER,
                                     VCREATEDBY                  USERS.USER_ID%TYPE,
                                     ...
                                     V_TC_ITEM_SET_ID            TEST_CATALOG.ITEM_SET_ID%TYPE,
                                     V_STU_GRADE                 STUDENT.GRADE%TYPE) AS

    V_IS_CURSOR_RETURNS_VALUE BOOLEAN := FALSE;
    V_ASSIGN_FORM             TEST_ROSTER.FORM_ASSIGNMENT%TYPE := V_FORM_ASSIGNMENT;
    
  BEGIN

    OPEN CUR_GET_STU_FOR_OK_3_8(ORG_NODE_ID, VPRODUCTID, V_TC_ITEM_SET_ID, V_STU_GRADE);
    LOOP
      FETCH CUR_GET_STU_FOR_OK_3_8 BULK COLLECT
      ...
      FOR I IN 1 .. V_ARRAY_OF_STUDENT.COUNT LOOP
        ...
        /*IF V_ARRAY_OF_STUDENT(I).SCREEN_READER IS NOT NULL THEN
          IF V_ARRAY_OF_STUDENT(I).SCREEN_READER = 'T' THEN
             V_ASSIGN_FORM := '1'; -- assign 'Form 1' of corres to the test to student with screen reader acco.
          END IF;
        END IF;*/
        
        IF ((V_STUDENT_LIST IS NOT NULL) AND (LENGTH(TRIM(V_STUDENT_LIST)) > 0)) THEN
          V_STUDENT_LIST := V_STUDENT_LIST || ',' || V_ARRAY_OF_STUDENT(I).STUDENT_ID;
        ELSE
      ...
      END LOOP;
      EXIT WHEN CUR_GET_STU_FOR_OK_3_8%NOTFOUND;
    END LOOP;

    CLOSE CUR_GET_STU_FOR_OK_3_8;
  ...
  END INSERT_TEST_ROSTER_FIXED;
  
  PROCEDURE INSERT_TEST_ROSTER_RR(VPRODUCTID                  INTEGER,
                                  VCUSTOMERID                 INTEGER,
                                  VCREATEDBY                  USERS.USER_ID%TYPE,
                                  VTESTADMINID                TEST_ADMIN.TEST_ADMIN_ID%TYPE,
                                  ORG_NODE_ID                 ORG_NODE.ORG_NODE_ID%TYPE,
  ...
  BEGIN
    ...
    FOR REC_GET_FORM_ASSIGNMENT IN CUR_GET_FORM_ASSIGNMENT(VTESTADMINID) LOOP
      V_ARRAY_FORM(FORM_COUNT) := REC_GET_FORM_ASSIGNMENT.FORM;
  ...
  PROCEDURE INSERT_SCHEDULABLE_UNITS_FIXED(VCUSTOMERID    INTEGER,
                                           RECTCITEMSETID CUR_GET_TCTEST_FOR_PRODUCT%ROWTYPE,
  ...
  END INSERT_SCHEDULABLE_UNITS_FIXED;
      ...
      IF ((V_STUDENT_LIST IS NOT NULL) AND (LENGTH(TRIM(V_STUDENT_LIST)) > 0)) THEN
        ...
        FOR I IN 1 .. V_ARRAY_FORM.COUNT LOOP
          FOR RECTDITEMSETID IN CRSGETTDTESTELEMENTLIST(RECTSITEMSETID.ITEM_SET_ID,
                                                        V_ARRAY_FORM(I)) LOOP
            V_IS_CURSOR_RETURNS_VALUE := TRUE;
            --dbms_output.put_line(RECTDITEMSETID.ITEM_SET_ID);
            EXECUTE IMMEDIATE 'insert into student_item_set_status
              (test_roster_id,
               item_set_id,
               completion_status,
                 ...
                 from (select distinct test_roster_id, cc.default_value
                        ...
                        where cc.customer_id = ' ||
                          ...
                          and cc.customer_configuration_name = ' || '''' ||
                              VROSTER_STATUS_FLAG || '''' ||
                              ' and test_admin_id = ' || VTESTADMINID ||
                              ' and form_assignment = ' || V_ARRAY_FORM(I) ||
                              ' and student_id in (' || V_STUDENT_LIST || ' )
                       union
                       select distinct test_roster_id, null as default_value
                         from test_roster
                        where test_admin_id = ' ||
  ...
  PROCEDURE SCHEDULE_SESSIONS_FOR_OK_3_8(INPUT_CUSTOMER_ID     INTEGER,
    ...
    V_DEPTT_CONTENT_AREA         VARCHAR2(20);
    V_DEPTT_GRADE                VARCHAR2(2);
    IS_SESSION_SCHEDULED       BOOLEAN := FALSE;
  ...
  BEGIN
    ...
    INITIALIZE_GLOBAL_ARRAYS; -- Initialize the global arrays 
              
    /* Setting product_id & test_window based on the input testing window */
    IF UPPER(INPUT_TESTING_WINDOW) = 'OPERATIONAL' THEN
         V_PRODUCT_ID := SPRING_FORM_3_8_PRODUCT_ID;
    END IF;                 
    ...
    FOR REC_SCHOOL_NODE IN CUR_FETCH_ALL_SCHOOL_NODES(INPUT_CUSTOMER_ID) LOOP
        ...
        --dbms_output.put_line(REC_SCHOOL_NODE.ORG_NODE_ID || '-' || REC_SCHOOL_NODE.ORG_NODE_NAME);
      ...
      IF V_CREATEDBY IS NOT NULL THEN
         ...
         FOR REC_DEPTT_NODE IN CUR_GET_DEPARTMENT_NODES(REC_SCHOOL_NODE.ORG_NODE_ID) LOOP
             ...
             V_IS_CURSOR_RETURNS_VALUE := FALSE;
             --DBMS_OUTPUT.PUT_LINE(REC_DEPTT_NODE.ORG_NODE_ID ||'-'|| REC_DEPTT_NODE.ORG_NODE_NAME);
             
             /* Getting grade value from department*/
             FOR I IN V_ARRAY_GRADE.FIRST .. V_ARRAY_GRADE.LAST LOOP
                 IF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, REPLACE(V_ARRAY_GRADE(I),0)) != 0 THEN
                    V_DEPTT_GRADE := V_ARRAY_GRADE(I);
                 END IF;
             END LOOP;
             /* Getting content area from department*/
             FOR J IN V_ARRAY_CONTENT_AREA.FIRST .. V_ARRAY_CONTENT_AREA.LAST LOOP
                 IF INSTR(UPPER(REC_DEPTT_NODE.ORG_NODE_NAME), V_ARRAY_CONTENT_AREA(J)) != 0 THEN
                    V_DEPTT_CONTENT_AREA := V_ARRAY_CONTENT_AREA(J);
                 END IF;
             END LOOP;
             
             --DBMS_OUTPUT.PUT_LINE(V_DEPTT_GRADE ||'-'||V_DEPTT_CONTENT_AREA);
             /*IF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, '6') != 0 THEN
                V_DEPTT_GRADE := '06';
             ELSIF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, '7') != 0 THEN
                V_DEPTT_GRADE := '07';
             ELSIF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, '8') != 0 THEN
                V_DEPTT_GRADE := '08';
             END IF;

             IF INSTR(UPPER(REC_DEPTT_NODE.ORG_NODE_NAME), 'READING') != 0 THEN
                V_DEPTT_CONTENT_AREA := 'READING';
             ELSIF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, 'MATHEMATICS') != 0 THEN
                V_DEPTT_CONTENT_AREA := 'MATHEMATICS';                
             ELSIF INSTR(REC_DEPTT_NODE.ORG_NODE_NAME, 'GEOGRAPHY') != 0 THEN
                V_DEPTT_CONTENT_AREA := 'GEOGRAPHY';
             END IF; 
             */   
                 
             FOR REC_TC_ITEMSETID IN CUR_GET_TCTEST_FOR_PRODUCT(V_PRODUCT_ID, V_CREATEDBY) LOOP
                 V_IS_CURSOR_RETURNS_VALUE := TRUE;
                 --DBMS_OUTPUT.PUT_LINE('OUTSIDE GRADE CHECK');
                 IF INSTR(V_DEPTT_GRADE, REC_TC_ITEMSETID.GRADE) != 0 THEN --alternate we can use grade as filter to fetch TC level Item_Sets
                   --DBMS_OUTPUT.PUT_LINE('INSIDE GRADE CHECK');
                   ...
                   /* Department Content Area i.e subject name matched with test :: will schedule session for students of this deptt. */
                  
                   IF INSTR(UPPER(V_TEST_NAME), V_DEPTT_CONTENT_AREA) != 0 THEN                        
                        --DBMS_OUTPUT.PUT_LINE('CONTENT AREA MATCHED WITH TEST NAME');                                          
                        IF V_PRODUCT_ID = SPRING_FORM_3_8_PRODUCT_ID THEN
                          
                          --need to add grade specific check also                                 
                          OPEN CUR_GET_STU_FOR_OK_3_8(REC_DEPTT_NODE.ORG_NODE_ID,
                                                      V_PRODUCT_ID,
                                                      REC_TC_ITEMSETID.ITEM_SET_ID,
                                                      V_DEPTT_GRADE);
                          FETCH CUR_GET_STU_FOR_OK_3_8 INTO V_STUDENT;
                          
                          IF (CUR_GET_STU_FOR_OK_3_8%FOUND) THEN
                             --DBMS_OUTPUT.PUT_LINE('STUDENT FOUND FOR GRADE '||V_DEPTT_GRADE);    
                            CLOSE CUR_GET_STU_FOR_OK_3_8;
                            ...
                            IF UPPER(SCHEDULING_TYPE) = 'ROUNDROBIN' THEN
                               --DBMS_OUTPUT.PUT_LINE('TODO:: ROUND ROBIN');
                               FORM_ASSIGNMENT_METHOD := 'roundrobin';
                               ...
                               INSERT_TEST_ADMIN(V_PRODUCT_ID,
                                                 ...
                                                 REC_SCHOOL_NODE.ORG_NODE_ID,
                                                 V_TEST_ADMIN_ID);
                                --DBMS_OUTPUT.PUT_LINE('TEST ADMIN DONE.... ');                                                                
                                V_REC_COUNT := 0;
                                INSERT_TEST_ROSTER_RR(V_PRODUCT_ID,
                                                      INPUT_CUSTOMER_ID,
                                                      V_CREATEDBY,
                                                      V_TEST_ADMIN_ID,
                                                      ...
                                                      REC_TC_ITEMSETID.ITEM_SET_ID,
                                                      V_DEPTT_GRADE);
                                --DBMS_OUTPUT.PUT_LINE('TEST ROSTER DONE.... ');                                                                          
                                INSERT_SCHEDULABLE_UNITS_RR(INPUT_CUSTOMER_ID,
                                                            REC_TC_ITEMSETID,
                                                            REC_SCHOOL_NODE.ORG_NODE_NAME,
                                                            V_TEST_ADMIN_ID,
                                                            V_TEST_ACCESS_CODE,
                                                            V_STUDENT_LIST);
                                --DBMS_OUTPUT.PUT_LINE('SISS DONE.... ');          
                                V_STUDENT_LIST := '';
                            
                            -- Commented as this feature is not required 
                            /*ELSIF UPPER(SCHEDULING_TYPE) = 'FIXED' THEN
                                FORM_ASSIGNMENT_METHOD := 'fixed';
                                ...
                                V_REC_COUNT := 0;
                                --need to compare for fixed scheduling
                                INSERT_TEST_ROSTER_FIXED(V_PRODUCT_ID,
                                                         INPUT_CUSTOMER_ID,
                                                         V_CREATEDBY,
                                                         V_TEST_ADMIN_ID,
                                                         REC_DEPTT_NODE.ORG_NODE_ID,
                                ...
                                INSERT_SCHEDULABLE_UNITS_FIXED(INPUT_CUSTOMER_ID,
                                                               REC_TC_ITEMSETID,
                                                               REC_SCHOOL_NODE.ORG_NODE_NAME,
                                                               V_TEST_ADMIN_ID,
                                                               V_TEST_ACCESS_CODE,
                          ...
                          ELSE
                            ...
                            V_TEST_ADMIN_NAME := NULL;                            
                          END IF;
                          
                          -- If a session is scheduled for the deptt. then logging relevant data for students of different grades present in that deptt...
                          IF IS_SESSION_SCHEDULED THEN
                            OPEN CUR_GET_STU_FOR_OTHER_GRADES(REC_DEPTT_NODE.ORG_NODE_ID,
                                                              V_PRODUCT_ID,
                                                              REC_TC_ITEMSETID.ITEM_SET_ID,
                                                              V_DEPTT_GRADE);
                            FETCH CUR_GET_STU_FOR_OTHER_GRADES INTO V_STU_OTHER_GRADES;
                            
                            IF (CUR_GET_STU_FOR_OTHER_GRADES%FOUND) THEN          
                              CLOSE CUR_GET_STU_FOR_OTHER_GRADES;
                              dbms_output.put_line('Logging into log table ::' || V_LOG_ID);
                              LOG_MESSAGE(V_LOG_ID,
                                          V_STATUS_WARN,
                                          '[ '|| V_STU_OTHER_GRADES.LOGGIN_INFO ||' ]' || '.. The above imported students are not scheduled in test session ['|| V_TEST_ADMIN_NAME ||'] for department [' || REC_DEPTT_NODE.ORG_NODE_NAME
                                          || ' - ' || REC_DEPTT_NODE.ORG_NODE_ID ||'] as they do not belong to the department grade [' || V_DEPTT_GRADE || '].');
                            ELSE
                              CLOSE CUR_GET_STU_FOR_OTHER_GRADES;
                            END IF; 
                          END IF; 
                   ...
                   ELSE
                       V_TEST_NAME := NULL;  
                   END IF; -- END OF CA CHECK                   
                 
                 /*ELSE
                     dbms_output.put_line('Need to log for students of different grades present in a particular grade...'); */
                 END IF;-- END OF GRADE CHECK    
             END LOOP;
  ...
  END SCHEDULE_SESSIONS_FOR_OK_3_8;
