...
public class CreateFiles2ndEdition {
	...
	public void writeToText() throws IOException, FFPojoException,SQLException, Exception {
		...
		if(!"".equals(Configuration.getMarkAsStudnetExported())
			...
			logger.info("Marking process is successfully completed for file :: ["+localFilePath+File.separator+fileName.toString() +" ].");
		}
	}
	...
	private void getTestRoster(Connection oascon, Connection irscon, SimpleCache cache) throws Exception {
		...
		try{
			...
			if (!"".equals(Configuration.getMarkAsStudnetExported())
					&& "true".equalsIgnoreCase(Configuration.getMarkAsStudnetExported())){
				Constants.MODIFIED_QUERY_TO_FETCH_ROSTER += Constants.STUDENT_EXPORT_CHECK_CONDITION; 
			}			
			...
			while (rs.next()){
				...
				//:prepare IN clause string for hand scoring validate query
		...
		if(cache.size() != 0){
			...
			if (!"".equals(Configuration.getMarkAsStudnetExported())
				...
				if (inCompletedCount != 0) {
					...
					logger.info("Unscored rosters are :: "+CreateFiles2ndEdition.unexportedRosters);
				}
			...
			if(cache.size() != 0){
				...
				generateStudentDetails(oascon, irscon, cache,
						studentIds.toString(), rosterStud.toString(), rosterIds
								.toString(), sessionStuIds.toString(), adminIds
								.toString());
				logger.info("8 level of Caching process completed... #Total Cache Size :: "
								+ cache.size());
			}
		}
	}
	...
	private void populatedRequiredData(SimpleCache cache, StringBuilder studentIds,
		...
		if(cache.size()!= 0){
			...
			CreateFiles2ndEdition.exportedRosters = rosterIds.toString();
		}
	}
...
//						}
	...
	private void markExportedFlagAsTrue() throws Exception {
		...
		try {
			oascon = SqlUtil.openOASDBconnectionForResearch();
			String[] str = CreateFiles2ndEdition.exportedRosters.split("#");
			for (int i = 0; i < str.length; i++) {
