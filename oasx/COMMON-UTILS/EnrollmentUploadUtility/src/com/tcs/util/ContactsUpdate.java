...
public class ContactsUpdate {
	private static final Logger logger = Logger.getLogger(NetworkUpdate.class);
	...
	private void processRecords(List<Contects> record,
		...
		for (Contects indvRecord : record) {
			try {
				...
				siteSurveyId = CSVFileReader.getSiteSurveyId(
						this.rtsCustomerId, schoolId, districtId, connection);
				switch (siteSurveyId) {
				case 0:
					logger.info("Given Contact Update combination does not matches in DB :district id ["
							+ indvRecord.getDistrictNumber()
							+ "], school id ["
							+ indvRecord.getSchoolNumber()
							+ "]. Skipping processing of this row.");
					break;
				default:
					UpdateContactHelper contactHelpher = new UpdateContactHelper(
							preparedStatement);
					contactHelpher.processEachRecordForOkhlahama(siteSurveyId,
							indvRecord, String.valueOf(this.rtsCustomerId));
