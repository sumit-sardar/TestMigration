CREATE OR REPLACE PROCEDURE ASSIGN_FIXED_SET_OF_FORM_GA AS
  ...
  CURSOR CUR_GET_STUDENT_DETAILS IS
    SELECT STUDENT_ID, ATTEMPTED_COUNTER
      FROM ROSTER_FORM_REASSIGN_INPUT
     WHERE JOB_STATUS = 'Prepared'
       AND ATTEMPTED_COUNTER < V_MAX_ATTEMPT_COUNTER
     ORDER BY STUDENT_ID;
  ...
  **/
  CURSOR CUR_GET_TEST_ROSTER_FORM(V_STUDENT_ID TEST_ROSTER.STUDENT_ID%TYPE) IS
    SELECT TR.TEST_ROSTER_ID,
     ...
     WHERE TA.PRODUCT_ID IN (V_GA_PRODUCT_ID, V_GA_MID_MONTH_PRODUCT_ID, V_GA_EOG_2015_PRODUCT_ID, V_GA_EOC_2015_PRODUCT_ID)
...
BEGIN
  FOR COL IN CUR_GET_STUDENT_DETAILS LOOP
    BEGIN
      V_ROSTER_COUNT := 0;
      ...
      IF V_ROSTER_COUNT > 0 THEN
        ...
        UPDATE ROSTER_FORM_REASSIGN_INPUT
           SET UPDATED_DATE_TIME    = SYSDATE,
               ...
               JOB_STATUS           = 'Executed',
               REASON               = 'Re-assign form of all rosters those are in SC/NT and have non screen reader GA Test_forms',
      ...
      ELSE
        ...
        UPDATE ROSTER_FORM_REASSIGN_INPUT
           SET UPDATED_DATE_TIME    = SYSDATE,
               UPDATED_ROSTER_COUNT = V_ROSTER_COUNT,
               JOB_STATUS           = 'Blocked',
               REASON               = 'Some other Test_forms are assigned or other products of GA customer Blocked',
               ATTEMPTED_COUNTER    = ATTEMPTED_COUNTER + 1
         WHERE STUDENT_ID = COL.STUDENT_ID
           AND JOB_STATUS = 'Prepared';
      END IF;
