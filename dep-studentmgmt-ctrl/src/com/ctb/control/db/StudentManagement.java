...
{ 
   ...
   //Added for TABE BAUM - 028
    ...
    @JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME, STUDENTIDNUMBER, ORGNAMELIST, OAS_UTILS.FILTER_COMPLETED_SUBTEST_STR(ROSTERID, CONTENTAREASTRING) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE AS ADMINISTRATIONDATE, DEFAULTSCHEDULER FROM (SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, STD.GRADE AS GRADE, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, ADM.TEST_ADMIN_ID AS TESTADMINID, NODE.ORG_NODE_NAME AS ORG, TC.TEST_NAME AS TESTNAME, STD.EXT_PIN1 AS STUDENTIDNUMBER, ROS.FORM_ASSIGNMENT AS FORM, ROS.START_DATE_TIME AS LOGIN_START_DATE, USR.USER_NAME AS DEFAULTSCHEDULER FROM ORG_NODE_ANCESTOR       ONA, ORG_NODE_STUDENT        ONS, STUDENT                 STD, ORG_NODE                NODE, TEST_ROSTER             ROS, TEST_ADMIN              ADM, TEST_CATALOG            TC, STUDENT_ITEM_SET_STATUS SISS, USERS                   USR, PRODUCT                 PROD WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND PROD.PARENT_PRODUCT_ID = {productId} AND PROD.PRODUCT_ID = ADM.PRODUCT_ID AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID AND ADM.CREATED_BY = USR.USER_ID AND STD.CUSTOMER_ID = ROS.CUSTOMER_ID  AND STD.CUSTOMER_ID = NODE.CUSTOMER_ID  AND ADM.CUSTOMER_ID = ROS.CUSTOMER_ID  AND STD.CUSTOMER_ID = ONS.CUSTOMER_ID  AND ONS.ORG_NODE_ID = NODE.ORG_NODE_ID AND ROS.START_DATE_TIME >= (SYSDATE - 365) AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y')) DERIVED1 GROUP BY ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER ORDER BY LOGINID) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
            arrayMaxLength = 0, fetchSize = 100)
