CREATE OR REPLACE PROCEDURE SP_INTERRUPTER is
    /**********************************************************************
	*  Modified On     BY          Reason for Modification
	*----------------------------------------------------------------------
	*  07-Jan-2015     SBASA       To ignore BMTSYNC Customer Ids
    *
	*
	**********************************************************************/
...
CURSOR C_ROSTERS IS
       ...
       SELECT TR.TEST_ROSTER_ID, TR.Customer_Id FROM TEST_ROSTER TR,
       ...
       AND (TR.UPDATED_DATE_TIME < SYSDATE - 3/1440 AND TR.TEST_COMPLETION_STATUS in ('IP', 'SP'))
       AND MAX_TIME < SYSDATE - 3/1440
       and ((TR.updated_date_time > sysdate - 3 and tr.test_admin_id not in (81821,81822,81823,81824,81831,81832,81833))
       or (TR.updated_date_time > sysdate - 0.5 and tr.test_admin_id in (81821,81822,81823,81824,81831,81832,81833)))
	   and tr.Customer_id NOT IN (SELECT BC.Customer_id FROM BMTSYNC_CUSTOMER BC)
       UNION ALL
       SELECT TR.TEST_ROSTER_ID, TR.Customer_Id
       FROM TEST_ROSTER TR
       WHERE TR.UPDATED_DATE_TIME < SYSDATE - 3/1440 AND TR.TEST_COMPLETION_STATUS in ('IP', 'SP')
       AND NOT EXISTS (select * from item_response where test_roster_id = tr.test_roster_id)
       and TR.updated_date_time > sysdate - 3
...
set TERMOUT on
PROMPT SP_INTERRUPTER compiled
