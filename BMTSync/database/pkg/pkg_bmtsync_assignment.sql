...
CREATE OR REPLACE PACKAGE BODY PKG_BMTSYNC_ASSIGNMENT AS
	...
	*/
	PROCEDURE updateAssignmentAPIStatus(
        ...
        vCustomerId           BMTSYNC_Assignment_Status.Customer_Id%TYPE;
		vTestRosterId         BMTSYNC_Assignment_Status.Roster_Id%TYPE;
	BEGIN
	    SELECT Customer_Id, Roster_Id, No_Of_Attempts INTO vCustomerId, vTestRosterId, vNoOfAttempts
  ...
  INSERT INTO BMTSYNC_Assignment_Status (Customer_Id, Test_Admin_ID, Student_ID, Roster_Id, App_Name, Exported_ON, Export_Status, Error_Code, Error_Message, Next_Retry_DateTime)
    VALUES (vCustomerId, pTestAdminID, pStudentID, vTestRosterId, pAppName, SYSDATE, pExportStatus, '', '', SYSDATE);
