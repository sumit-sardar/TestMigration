    	...
    	try {
    		...
    		for(int k=0;k<manifests.length;k++) {
	    		...
	    		if(k==0) {
		    		...
		    		stmt2.setString(1, manifest.getSdsId());
		    		stmt2.setString(2, manifest.getRosterCompletionStatus());
		    		stmt2.setInt(3, manifest.getRosterRestartNumber());
		    		if(manifest.getRosterStartTime() == 0) {
			    		...
			    		stmt2.setTimestamp(5, new Timestamp(manifest.getRosterStartTime()));
		    		}
		    		stmt2.setTimestamp(6, new Timestamp(System.currentTimeMillis()));
		    		if(manifest.getRosterEndTime() == 0) {
		    			stmt2.setTimestamp(7, null);
		    		} else {
			    		stmt2.setTimestamp(7, new Timestamp(manifest.getRosterEndTime()));
		    		}
		    		stmt2.setInt(8, manifest.getRosterLastMseq());
		    		stmt2.setInt(9, manifest.getRosterCorrelationId());
		    		Integer seed = null;
		    		if(subtests[0] != null && "Y".equals(subtests[0].getRandomDistractorStatus())) {
		    			seed = new Integer(manifest.getRandomDistractorSeed());
		    		}
		    		if(seed != null && !"".equals(seed)) {
		    			stmt2.setInt(10, seed);
		    		} else {
		    			stmt2.setObject(10, null);
		    		}
		    		stmt2.setString(11,manifest.getTtsSpeedStatus());
		    		stmt2.setString(12, testRosterId);
		    		stmt2.executeUpdate();
		    		stmt2.close();
		    		stmt2 = null;
