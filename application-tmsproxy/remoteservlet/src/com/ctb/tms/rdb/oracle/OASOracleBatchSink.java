...
public class OASOracleBatchSink implements OASRDBBatchSink {	
	...
	static Logger logger = Logger.getLogger(OASOracleBatchSink.class);
	...
	public PreparedStatement getStoreResponseStatement(Connection conn) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
			...
			return conn.prepareStatement(STORE_RESPONSE_SQL);
	...
	public PreparedStatement getDeleteCRResponseStatement(Connection conn) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
			...
			return conn.prepareStatement(DELETE_CR_RESPONSE_SQL);
	...
	public PreparedStatement getStoreCRResponseStatement(Connection conn) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		return conn.prepareStatement(STORE_CR_RESPONSE_SQL);

		}


	public PreparedStatement getCRResponseExistsStatement(Connection conn) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {

		return conn.prepareStatement(CR_RESPONSE_EXISTS_SQL);
	...
	public void putItemResponse(Connection conn, ItemResponseData ird) throws NumberFormatException, SQLException, IOException, ClassNotFoundException {		
		...
		logger.debug("\n***** OASOracleBatchSink: putItemResponse: " + ird.getTestRosterId() + ", mseq: " + ird.getResponseSeqNum() + ", item: " + ird.getItemId() + ", item type: " + ird.getItemType() + ", response type: " + responseType + ", elapsed time: " + ird.getResponseElapsedTime() + ", response: " + response + ", CR response: " + CRresponse);
	}
	...
	public void putItemResponse(Connection conn, PreparedStatement storeResponseStatement, PreparedStatement deleteCRResponseStatement, PreparedStatement storeCRResponseStatement, ItemResponseData ird) throws NumberFormatException, SQLException, IOException, ClassNotFoundException {		
		...
		try {
			if(response != null && !"".equals(response.trim())) {
				if(responseType.equals(BaseType.IDENTIFIER.toString())) {
					logger.debug("Storing SR response");
					storeResponse(storeResponseStatement, testRosterId, ird.getItemSetId(), ird.getItemId(), response, ird.getResponseElapsedTime(), ird.getResponseSeqNum(), ird.getStudentMarked());
	            } else if(responseType.equals(BaseType.STRING.toString())) {
	            	logger.debug("Storing CR response");
	            	CRresponse = response;
	            	response = null;
	            	storeResponse(storeResponseStatement, testRosterId, ird.getItemSetId(), ird.getItemId(), response, ird.getResponseElapsedTime(), ird.getResponseSeqNum(), ird.getStudentMarked());
	                storeCRResponse(conn, deleteCRResponseStatement, storeCRResponseStatement, testRosterId, ird.getItemSetId(), ird.getItemId(), CRresponse, ird.getResponseElapsedTime(), ird.getResponseSeqNum(), ird.getStudentMarked(), ird.isAudioItem());
	            }
				//logger.info("Finished putItemResponse for roster " + testRosterId + ", mseq " + ird.getResponseSeqNum());
		    } else{
		    	logger.debug("Storing null response");
	...
	private void storeResponse(PreparedStatement stmt1, int testRosterId, int itemSetId, String itemId, String response, float duration, String mseq, String studentMarked) throws SQLException {
		logger.debug("\n***** OASOracleBatchSink: Called storeResponse for roster: " + testRosterId + ", item: " + itemId);
		//PreparedStatement stmt1 = null;
    	try {
			//stmt1 = con.prepareStatement(STORE_RESPONSE_SQL);
			...
			stmt1.addBatch();
			logger.debug("OASOracleBatchSink: Stored response record in DB for roster " + testRosterId + ", mseq " + mseq + ", response: " + response);
		} catch (SQLException e) {
			if(e.getMessage().indexOf("unique constraint") >= 0 ) {
    		...
    		for(int k=0;k<manifests.length;k++) {
	    		...
	    		for(int i=0;i<subtests.length;i++) {
	    			...
	    			if(!"SC".equals(subtest.getCompletionStatus())) {
		    			...
		    			stmt1.executeUpdate();
		    			logger.debug("OASOracleBatchSink: Updated subtest status for roster: " + testRosterId + ", subtest: " + subtest.getId() + ". Status is: " + subtest.getCompletionStatus());
		    			stmt1.close();
		    			stmt1 = null;
	    			}
	    		}
...
//		    		if(seed != null && !"".equals(seed)) {  // seed is an Integer
		    		...
		    		stmt2.close();
		    		stmt2 = null;
					logger.debug("OASOracleBatchSink: Updated roster status for roster: " + testRosterId + ". Status is: " + manifest.getRosterCompletionStatus());
					if(manifest.getTutorialTaken() != null) {
			    		...
			    		stmt4.close();
			    		stmt4 = null;
			    		logger.info("OASOracleBatchSink: Updated tutorial status for roster: " + testRosterId + ". Status is: " + manifest.getTutorialTaken());
					}
	    		}
    		}
    	} finally {
	...
	private void storeCRResponse(Connection conn, int testRosterId, int itemSetId, String itemId, String response, float duration, String mseq, String studentMarked, boolean audioItem) throws SQLException {
    	...
    	try {
			...
			logger.debug("OASOracleBatchSink: Stored CR response record in DB for roster " + testRosterId + ", mseq " + mseq);
		} catch (SQLException e) {
			if(e.getMessage().indexOf("unique constraint") >= 0 ) {
	...
	private void storeCRResponse(Connection conn, PreparedStatement stmt1, PreparedStatement stmt2, int testRosterId, int itemSetId, String itemId, String response, float duration, String mseq, String studentMarked, boolean audioItem) throws SQLException {
		//PreparedStatement stmt1 = null;
		//PreparedStatement stmt2 = null;
		PreparedStatement stmt3 = null;
    	try {
			...
			logger.debug("OASBatchOracleSink: Stored CR response record in DB for roster " + testRosterId + ", mseq " + mseq);
		} catch (SQLException e) {
			if(e.getMessage().indexOf("unique constraint") >= 0 ) {
				// do nothing, dupe response
			} else {
