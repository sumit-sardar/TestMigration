...
{ 
	...
	private static final String ACTIVE_ROSTERS_SQL = "select distinct stu.user_name as username, tr.password as password, upper(tais.access_code) as accesscode, sum(siss.item_Set_id) as manifestHash from test_roster tr, test_admin ta, test_admin_item_set tais, student stu, student_item_Set_Status siss, item_set_parent isp where tr.test_completion_status in ('SC', 'IN', 'IS', 'IP') and sysdate > (TA.LOGIN_START_DATE - 3) and sysdate < (TA.LOGIN_END_DATE + 3) and ((tr.updated_date_time > sysdate - 30 and ta.updated_date_time > sysdate - 30) OR (stu.user_name > 'pt-student1450000' and stu.user_name < 'pt-student1500000') OR (tr.updated_date_time is null and ta.updated_date_time > sysdate - 30))  and ta.test_admin_id = tr.test_admin_id and tais.test_admin_id = ta.test_admin_id and stu.student_id = tr.student_id and siss.test_roster_id = tr.test_roster_id and tais.item_set_id = isp.parent_item_Set_id and siss.item_set_id = isp.item_Set_id group by stu.user_name, tr.password, upper(tais.access_code)";
	...
	public StudentCredentials [] getActiveRosters(Connection con) {
    	...
    	try {
			...
			while (rs1.next()) {
				...
				creds.setManifestHash(rs1.getLong("manifestHash"));
				dataList.add(creds);
			}
			rs1.close();
