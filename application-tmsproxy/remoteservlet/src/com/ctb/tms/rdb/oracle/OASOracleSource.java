...
{ 
	...
	private static final String GET_CAT_PRIOR_SQL = "select distinct old_siss.test_roster_id as priorRosterId, curr_siss.item_set_id as subtestId, curr_iset.ads_ob_asmt_id as externalId, old_siss.ability_score as abilityScore from test_roster curr_ros, student_item_set_status curr_siss, item_set curr_iset, test_roster old_ros, student_item_set_status old_siss, item_set old_iset where curr_ros.test_roster_id = ? and curr_siss.test_roster_id = curr_ros.test_roster_id and old_ros.student_id = curr_ros.student_id and old_ros.test_roster_id < curr_ros.test_roster_id and old_siss.test_roster_id = old_ros.test_roster_id and curr_iset.item_set_id = curr_siss.item_set_id and old_iset.item_set_id = old_siss.item_set_id and old_siss.completion_status not in ('SC', 'NT') and curr_iset.sample = 'F' and old_iset.sample = 'F' and (old_iset.item_Set_name = curr_iset.item_Set_name or old_iset.item_set_name = 'TABE ' || curr_iset.item_Set_name ) order by old_siss.test_roster_id, curr_siss.item_set_id";
	...
	private static final String GET_ACTIVE_ADAPTIVE_ROSTERS_SQL = "select distinct " +
					"stu.user_name as username, " +
					"    tr.password as password, " +
					"   tais.access_code as accessCode, " +
					"   'F' as tmsUpdate, " +
					...
					"    test_admin_item_set tais " +
					"where " +
					"    stu.student_id = tr.student_id " +
					"    and adm.test_admin_id = tr.test_admin_id " +
					"   and tais.test_admin_id = tr.test_admin_id " +
					"    and adm.product_id = 8001 " +
					"    and tr.test_completion_status in ('SC', 'IN', 'IS')";
	...
	public StudentCredentials[] getActiveAdaptiveRosters(Connection con) {
    	...
    	int recordCount = 0;
    	try {
    		stmt1 = con.prepareStatement(GET_ACTIVE_ADAPTIVE_ROSTERS_SQL);
			ResultSet rs1 = stmt1.executeQuery();
			ArrayList<StudentCredentials> dataList = new ArrayList<StudentCredentials>();
			while (rs1.next()) {
				recordCount++;
	...
	public CATPriorData [] getCATPriorData(Connection con, int testRosterId) {
    	...
    	try {
			...
			ResultSet rs1 = stmt1.executeQuery();
			HashMap<String, CATPriorData> dataList = new HashMap<String, CATPriorData>();
			while (rs1.next()) {
				...
				dataList.put(String.valueOf(prior.getSubtestId()), prior);
			}
			rs1.close();
			return (CATPriorData[]) dataList.values().toArray(new CATPriorData[0]);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
