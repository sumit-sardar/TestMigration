...
{ 
	...
	//Fix defect 77482 truncated CR response
	private static final String RESTART_RESPONSES_SQL = "select  ir.item_id as itemId,  ir.response_seq_num as responseSeqNum,  ir.student_marked as studentMarked,  i.item_type as itemType,  isi.item_sort_order as itemSortOrder,  ir.response as response, decode (i.answer_area,'AudioItem', decode(length(icr.constructed_response),'','',ir.test_roster_id || '_' || ir.item_id), CASE WHEN DBMS_LOB.GETLENGTH(icr.constructed_response) > 4000 THEN NULL ELSE DBMS_LOB.SUBSTR(icr.constructed_response, 4000, 1) END ) as constructedResponseStr, CASE WHEN i.answer_area = 'AudioItem' THEN NULL ELSE (CASE WHEN DBMS_LOB.GETLENGTH(icr.constructed_response) > 4000 THEN icr.constructed_response ELSE null END) END AS constructedResponseClob, ir.response_elapsed_time as responseElapsedTime,  decode(ir.response, i.correct_answer, 1, 0) as score,  i.ads_item_id as eid, decode(i.answer_area,'AudioItem','T','F') as audioItem from  item_response ir,  item i,  item_response_cr icr,  item_set_item isi where  ir.test_roster_id = ?  and ir.item_set_id = ?  and ir.item_id = i.item_id  and ir.item_set_id = isi.item_set_id  and ir.item_id = isi.item_id  and ir.response_seq_num =  (select  max(ir1.response_seq_num)  from  item_response ir1  where  ir1.item_set_id = ir.item_set_id  and ir1.item_id = ir.item_id  and ir1.test_roster_id = ir.test_roster_id) \t  AND ir.test_roster_id = icr.test_roster_id (+) \t  AND ir.item_set_id = icr.item_set_id (+) \t  AND ir.item_id = icr.item_id (+) order by  ir.response_seq_num asc"; //isi.item_sort_order";
	...
	private static final String MARK_ACTIVE_ROSTERS_SQL = "" +
			"		update prepop_table prepop set " +
			"			prepop.node_id = ? " +
			"		where " +
			"			prepop.node_id is null " +
			"			and prepop.test_roster_id in (" +
			"				select distinct " +
			"					test_roster_id from (" +
			"						select distinct " +
			"							test_roster_id " +
			...
			"                    test_admin ta " +
			"                where " +
			"                    tr.test_roster_id = prepop.test_roster_id " +
	...
	"                    and nvl(p.delivery_client_id , 99 ) <> " + TestDeliveryContextListener.BMT_DELIVERY_CLIENT_ID + " "+
	"                    and sysdate > (TA.LOGIN_START_DATE - 1) " + 
	"                    and sysdate < (TA.LOGIN_END_DATE + 1) " + 
	"                    and tr.activation_status in ('AC', 'IN') " +  
	"                    and ta.activation_status = 'AC' " + 
	...
	public void markActiveRosters(Connection con, String clusterName, int clusterId, int nodeid) {
    	...
    	try {
    		...
    		if(clusterName.endsWith("DR")) {
    			prepopTable = "TMS_SEC_CACHE_PREPOP";
    		}
        ...
        if(authData != null) {    
	        ...
	        if(accomData != null){
		        if (accomData.getScreenReader() != null && accomData.getScreenReader().equals("T")) {
	                String speechControllerFlag = accomData.getSpeechSpeedControl();
	                if (speechControllerFlag != null && "T".equals(speechControllerFlag)) {
	                    if (authData.getTtsSpeedStatus() != null) { 
	                    	loginResponse.setTtsSpeedValue(authData.getTtsSpeedStatus());
	                    } else {
	                       loginResponse.setTtsSpeedValue(Constants.SpeechController.DEFAULT_TTS_SPEED_VALUE );   
	                    }
	                }
	            }
	        }
...
//	                    }
    ...
    public static void setManifestDataInResponse(LoginResponse response, ManifestData [] manifestData) throws SQLException {
        ...
        ManifestData[] newManifestData = null;
        ...
        if(response.getRestartNumber().intValue() > 0) {
	        ...
	        newManifestData = (ManifestData []) a.toArray(new ManifestData[0]);
        } else {
        	newManifestData = manifestData;
        }
        
        for(int i=0;i<newManifestData.length;i++) {
        	ManifestData data = newManifestData[i];
        	manifest.setBlockContentDownload("T".equals(data.getBlockDownload())?"true":"false");
        	/*	if(response.getRestartFlag() && "T".equals(isUltimateAccessCode) 
	            ...
	            if(data.getAdminForceLogout().equals("T") &&
	                ((i >= manifestData.length - 1) || (data.getScoParentId() != newManifestData[i+1].getScoParentId()))) {
	            ...
	            sco.setIsSample("T".equals(data.getIsSample()) ? true : false);
	...
	public com.ctb.tms.bean.login.Manifest[] getManifest(Connection con, String testRosterId) {
    	...
    	try {
			...
			while (rs1.next()) {
    			...
    			manifest.setIsSample(rs1.getString("issample"));
	...
	private static AccommodationsData getAccommodations(Connection con, int testRosterId) {
    	...
    	try {
			...
			if (rs1.next()) {
				...
				data.setSpeechSpeedControl(rs1.getString("speechSpeedControl"));
...
//				response.setConstructedResponse(rs1.getString("constructedResponse"));
				...
				//Fix defect 77482 truncated CR response
				Clob constructedResponseClob = rs1.getClob("constructedResponseClob");
				if (constructedResponseClob != null)
				{
					response.setConstructedResponse(clobToString(constructedResponseClob));
				}
				else
				{
					response.setConstructedResponse(rs1.getString("constructedResponseStr"));
				}
