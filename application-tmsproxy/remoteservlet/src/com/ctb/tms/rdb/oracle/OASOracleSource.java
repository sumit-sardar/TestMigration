...
{ 
	...
	private static final String MARK_ACTIVE_ROSTERS_SQL = "update prepop_table set node_id = ? where node_id is null and test_roster_id in (select distinct test_roster_id from (select distinct test_roster_id from prepop_table where node_id is null) where rownum <= " + TestDeliveryContextListener.batchSize + ") ";
