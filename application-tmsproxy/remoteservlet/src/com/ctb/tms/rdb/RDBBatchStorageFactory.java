...
import com.ctb.tms.rdb.oracle.OASOracleBatchSink;
import com.ctb.tms.rdb.oracle.OASOracleSource;

public class RDBBatchStorageFactory {
	...
	private static OASRDBBatchSink oasBatchSink;
	...
	public static OASRDBBatchSink getOASBatchSink() {
		if(oasBatchSink == null) {
			if (ORACLE.equals(oassink)) {
				logger.info("*****  Using OASOracleBatchSink");
				oasBatchSink= new OASOracleBatchSink();
			} else {
				// default to Oracle
				logger.info("*****  Using OASOracleBatchSink");
				oasBatchSink= new OASOracleBatchSink();
