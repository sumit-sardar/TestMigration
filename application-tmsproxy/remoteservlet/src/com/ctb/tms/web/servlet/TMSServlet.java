...
public class TMSServlet extends HttpServlet {
	...
	private String feedback(String xml) throws XmlException, IOException, ClassNotFoundException {
        ...
        for(int i=0;i<feedback.length;i++) {
            ...
            feedbackResponse.getTitle().getScoArray()[i].setTitle(feedback[i].getProduct() + "::" + feedback[i].getTestTitle() + "::" + feedback[i].getTitle() + "::" + feedback[i].getForm() + "::" + feedback[i].getLevel() + "::" + feedback[i].getScoOrder() + "::" + feedback[i].getCompletionStatus());
	...
	private String save(HttpServletResponse response, String xml) throws Exception {
		...
		if(locatorComplete) {
    		...
    		if(nextScoIndex < manifestData.length) {
	        	...
	        	saveResponse.getTsdArray(0).getNextSco().setId(nextSco.getId());
	        	logger.warn("Get Re-selected next sco due to auto-locator: " + saveResponse.getTsdArray(0).getNextSco().getId());
    		}
    	}
