...
if [[ $1 == '-jmx' ]]; then
    JMXPROPERTIES="-Dtangosol.coherence.cacheconfig=${COHERENCE_HOME}/bin/coherence-cache-config.xml -Dcom.sun.management.jmxremote -Dtangosol.coherence.management=all -Dtangosol.coherence.management.remote=true"
    shift
fi
