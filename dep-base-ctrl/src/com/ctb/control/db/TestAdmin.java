...
{ 
	...
	@JdbcControl.SQL(statement =" select distinct ta.test_admin_id as testAdminId, ta.customer_id as customerId, ta.creator_org_node_id as creatorOrgNodeId, ta.test_admin_name as testAdminName, tr.test_roster_id as testRosterId,  ta.item_set_id as itemSetId,  tc.test_catalog_id as testCatalogId , tc.test_name as testName, ta.access_code as accessCode, ta.created_by as createdBy, ta.session_number as sessionNumber, concat(concat(usr.last_name, ', '), concat(usr.first_name, concat(' ', usr.MIDDLE_NAME))) as scheduler from test_admin ta, test_roster tr, test_catalog tc, users usr, customer_configuration cc where ta.test_admin_id = tr.test_admin_id and tc.test_catalog_id = ta.test_catalog_id and usr.user_id = ta.created_by and ta.activation_status = 'AC' and ta.test_admin_status = 'CU' and ta.customer_id = cc.customer_id and cc.customer_configuration_name in ('Allow_Reopen_Subtest', 'Allow_User_Reset_Subtest') and cc.default_value = 'T' {sql: searchCriteria}")
