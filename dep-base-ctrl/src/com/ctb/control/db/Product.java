...
TestProduct [] getTestCatalogForUser(String userName) throws SQLException;
    ...
    @JdbcControl.SQL(statement = "SELECT CATALOG.ITEM_SET_ID FROM PRODUCT PROD, TEST_CATALOG CATALOG, USERS, USER_ROLE UROLE, ORG_NODE_ANCESTOR ONA, ORG_NODE_TEST_CATALOG ONTC WHERE PROD.PARENT_PRODUCT_ID = {productId} AND PROD.PRODUCT_ID = CATALOG.PRODUCT_ID AND CATALOG.TEST_LEVEL = {level} AND USERS.USER_NAME = {userName} AND UROLE.USER_ID = USERS.USER_ID AND ONA.ANCESTOR_ORG_NODE_ID = UROLE.ORG_NODE_ID AND ONTC.ORG_NODE_ID = ONA.ORG_NODE_ID AND ONTC.PRODUCT_ID = PROD.PRODUCT_ID",
            arrayMaxLength = 1)
