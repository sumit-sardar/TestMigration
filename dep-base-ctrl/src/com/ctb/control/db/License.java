...
{ 
  ...
  // TABE BAUM 10: For updating the edited available license field value in manage license page
   ...
   // TABE BAUM 10 : Inserting license details into database for a particular organization who's entry is not there in the database table
      ...
      */
       //CustomerLicense change
     @JdbcControl.SQL(statement = "insert into customer_product_license     (customer_id,  product_id,  available, reserved,  consumed, subtest_model,   email_notify_flag,  license_after_last_purchase,order_index,order_number,po_text,license_period_start,license_period_end ) values ( {customerLicense.customerId},  {customerLicense.productId}, {customerLicense.available}, {customerLicense.reservedLicense}, {customerLicense.consumedLicense}, {customerLicense.subtestModel} ,  'T',  {customerLicense.available},seq_license_po_index.nextval,{customerLicense.orderNumber},{customerLicense.purchaseOrder},{customerLicense.licenseperiodStartdate},{customerLicense.licenseperiodEnd} )")
...
//     CustomerLicense getLASCustomerTopNodeData(Integer customerId, Integer productId) throws SQLException;
     ...
     @JdbcControl.SQL(statement = "UPDATE CUSTOMER_ORGNODE_LICENSE CONL SET CONL.LICENSE_AFTER_LAST_PURCHASE = {afterLastPurchase} , CONL.AVAILABLE = {available},CONL.EMAIL_NOTIFY_FLAG='T'  WHERE CONL.CUSTOMER_ID = {customerId}  AND CONL.PRODUCT_ID = {productId}  AND CONL.ORG_NODE_ID = {orgNodeId}")
