...
{ 
     ...
     */
    @JdbcControl.SQL(statement = "(select  message.DISPLAY_DATE_TIME as displayDateTime,  message.CREATED_DATE_TIME as createdDateTime,  message.MESSAGE as message,  message.PRIORITY_VALUE as priorityValue,  message.CREATED_BY as createdBy,  message.START_DATE as startDate,  message.END_DATE as endDate,  message.ACTIVATION_STATUS as activationStatus,  message.BROADCAST_MESSAGE_LOG_ID as broadcastMessageLogId,  message.APPLICATION as application,  message.MESSAGE_TYPE as messageType  from  broadcast_message_log message  where  message.application in ('OAS', 'ALL')  and message.activation_status = 'AC'  and sysdate > message.start_date and sysdate < message.end_date  and message.product_id is null)  union  (select  message.DISPLAY_DATE_TIME as displayDateTime,  message.CREATED_DATE_TIME as createdDateTime,  message.MESSAGE as message,  message.PRIORITY_VALUE as priorityValue,  message.CREATED_BY as createdBy,  message.START_DATE as startDate,  message.END_DATE as endDate,  message.ACTIVATION_STATUS as activationStatus,  message.BROADCAST_MESSAGE_LOG_ID as broadcastMessageLogId,  message.APPLICATION as application,  message.MESSAGE_TYPE as messageType  from  broadcast_message_log message  where  message.application in ('OAS', 'ALL')  and message.activation_status = 'AC'  and sysdate > message.start_date and sysdate < message.end_date  and {sql: searchCriteria})  order by priorityValue, displayDateTime desc",
                     arrayMaxLength = 100000)
