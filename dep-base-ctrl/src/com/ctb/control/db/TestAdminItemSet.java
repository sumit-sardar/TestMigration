...
{ 
    ...
    */ 
    ...
    @JdbcControl.SQL(statement = " select ta.test_admin_id as testAdminId, tais.item_set_id as itemSetId, ta.customer_id as customerId , ta.creator_org_node_id as creatorOrgNodeId,  its.item_set_name as itemSetName, tais.item_set_order as itemSetOrder, tais.access_code as accessCode, tais.item_set_form as itemSetForm from test_admin ta, test_admin_item_set tais, item_set its, customer_configuration cc where ta.activation_status = 'AC' and ta.test_admin_status = 'CU' and ta.test_admin_id = tais.test_admin_id and tais.item_set_id = its.item_set_id and ta.customer_id = cc.customer_id and cc.customer_configuration_name = 'Allow_Reopen_Subtest' and cc.default_value = 'T' and ta.test_admin_id in (select distinct tais.test_admin_id from test_admin_item_set tais where upper(tais.access_code) = upper({accessCode})) order by tais.item_set_order",
            arrayMaxLength = 100000)
