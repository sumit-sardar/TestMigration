...
public interface CRScoring extends JdbcControl {
	...
	//score by student: all student list from test admin
	 @JdbcControl.SQL(statement = "select distinct ros.TEST_ROSTER_ID   as testRosterId, ros.TEST_ADMIN_ID   as testAdminId, tcsc.test_completion_status_desc testCompletionStatusDesc, ros.VALIDATION_STATUS   as validationStatus, stu.grade   as grade, ros.STUDENT_ID   as studentId, ros.CUSTOMER_ID as customerId, ros.SCORING_STATUS as scoringStatus, ros.FORM_ASSIGNMENT as formAssignment, stu.first_name as firstName, stu.middle_name  as middleName,  stu.last_name  as lastName,  stu.ext_pin1 as extPin1,  stu.user_name   as userName,  tad.item_set_id  as itemSetIdTC from test_roster   ros, student  stu,  test_completion_status_code tcsc, test_admin   tad, org_node   node, org_node_student ons, org_node_ancestor ona, users   users,  user_role   urole  where tad.test_admin_id = {testAdminId}   and tad.test_admin_id = ros.test_admin_id    and stu.student_id = ros.student_id    and tcsc.test_completion_status in ('CO', 'IS', 'IC')    and tcsc.test_completion_status = ros.TEST_COMPLETION_STATUS    and stu.activation_Status = 'AC'    and ros.activation_status = 'AC'    and ros.validation_status in ('VA', 'PI')    and users.user_name = {userName}    and users.user_id = urole.user_id    and urole.activation_status = 'AC'    and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id    and ona.ORG_NODE_ID = ons.ORG_NODE_ID    and ons.org_node_id = ona.org_node_id    and node.org_node_id = ons.org_node_id    and stu.student_id = ons.student_id ",
