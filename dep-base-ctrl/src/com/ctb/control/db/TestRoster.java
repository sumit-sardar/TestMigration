...
{ 
    ...
    */ 
    ...
    @JdbcControl.SQL(statement = "select  ros.TEST_ROSTER_ID as testRosterId,  ros.TEST_ADMIN_ID as testAdminId,  ros.CREATED_DATE_TIME as createdDateTime,  ros.START_DATE_TIME as startDateTime,  ros.COMPLETION_DATE_TIME as completionDateTime,  ros.TEST_COMPLETION_STATUS as testCompletionStatus,  ros.VALIDATION_STATUS as validationStatus,  ros.VALIDATION_UPDATED_BY as validationUpdatedBy,  ros.VALIDATION_UPDATED_DATE_TIME as validationUpdatedDateTime,  ros.VALIDATION_UPDATED_NOTE as validationUpdatedNote,  ros.OVERRIDE_TEST_WINDOW as overrideTestWindow,  ros.PASSWORD as password,  ros.STUDENT_ID as studentId,  ros.CREATED_BY as createdBy,  ros.UPDATED_BY as updatedBy,  ros.ACTIVATION_STATUS as activationStatus,  ros.UPDATED_DATE_TIME as updatedDateTime,  ros.CUSTOMER_ID as customerId,  ros.TUTORIAL_TAKEN_DATE_TIME as tutorialTakenDateTime,  ros.CAPTURE_METHOD as captureMethod,  ros.SCORING_STATUS as scoringStatus,  ros.ORG_NODE_ID as orgNodeId,  ros.FORM_ASSIGNMENT as formAssignment,  stu.first_name as firstName,  stu.middle_name as middleName,  stu.last_name as lastName,  stu.ext_pin1 as extPin1,  stu.user_name as userName,  ros.CUSTOMER_FLAG_STATUS as customerFlagStatus, org.org_node_name as className from  test_roster ros, student stu, org_node_ancestor anc, org_node org where  ros.test_admin_id = {testAdminId}  and anc.ancestor_org_node_id = {orgNodeId}  and anc.org_node_id = ros.org_node_id  and stu.student_id = ros.student_id and ros.org_node_id = org.org_node_id  and (stu.activation_Status = 'AC' or decode(decode(ros.test_completion_status, 'NT', 'SC', ros.test_completion_status), 'SC', 'F', 'T') = 'T') and ros.ACTIVATION_STATUS != 'IN' ",
                     arrayMaxLength = 100000)
    ...
    Integer [] getTSItemSetIds(Integer testRosterId) throws SQLException;
  	//OrgNode Hierarchy For a user  
    @JdbcControl.SQL(statement = "select org.org_node_id      as orgNodeId, org.org_node_name    as orgNodeName, anc.org_node_id      as parentOrgNodeId, anc.org_node_name    as parentOrgNodeName, ona.number_of_levels as categoryLevel from users                u, test_admin_user_role tar, test_roster          tr, org_node             org, org_node_ancestor    ona, org_node             anc where tar.user_id = u.user_id and tar.test_admin_id = tr.test_admin_id and tr.activation_status = 'AC' and tr.test_admin_id = {testAdminId} and tr.org_node_id = org.org_node_id and org.org_node_id = ona.org_node_id and anc.org_node_id = ona.ancestor_org_node_id and anc.activation_status = 'AC' and u.user_name = {userName} and ona.number_of_levels < = ((select max(onc.category_level)as hierarchylevel from users             u, user_role         ur, org_node_category onc, org_node          userorg where ur.user_id = u.user_id and ur.org_node_id = userorg.org_node_id and userorg.customer_id = onc.customer_id and ur.activation_status = 'AC' and onc.activation_status = 'AC' and userorg.activation_status = 'AC' and u.user_name = {userName}) - (select onc.category_level as usertoplevel from users             u, user_role         ur, org_node_category onc, org_node          userorg where ur.user_id = u.user_id and ur.org_node_id = userorg.org_node_id and ur.activation_status = 'AC' and userorg.org_node_category_id = onc.org_node_category_id and onc.activation_status = 'AC' and userorg.activation_status = 'AC' and u.user_name = {userName} and userorg.org_node_id = {orgNodeId})) and upper(anc.org_node_name) not in ('ROOT', 'CTB') AND EXISTS (SELECT 1 FROM ORG_NODE_ANCESTOR ONAI WHERE ONAI.ORG_NODE_ID = ona.org_node_id AND ONAI.ANCESTOR_ORG_NODE_ID = {orgNodeId}) group by org.org_node_id, org.org_node_name, anc.org_node_id, anc.org_node_name, ona.number_of_levels order by org.org_node_id, ona.number_of_levels desc")
    ClassHierarchy [] getOrgNodeHierachy(String userName, Integer testAdminId, Integer orgNodeId) throws SQLException;
    //OrgNodeIds For a user  
    ...
    */ 
    ...
    /**
     * @jc:sql statement::
     * DELETE FROM TABE_CCSS_ROSTER_LEVEL WHERE TEST_ROSTER_ID IN (SELECT TEST_ROSTER_ID FROM TEST_ROSTER WHERE TEST_COMPLETION_STATUS IN ('SC', 'NT') AND TEST_ADMIN_ID = {testAdminId})::
     */
