...
//END -ADDED for for toggling values of exemption and absent
	...
	@JdbcControl.SQL(statement ="update student_item_set_status set completion_status = 'CO', START_DATE_TIME = sysdate, COMPLETION_DATE_TIME = sysdate, RAW_SCORE = null, MAX_SCORE= null, UNSCORED= null, RECOMMENDED_LEVEL= null, ABILITY_SCORE= null,  SEM_SCORE= null, OBJECTIVE_SCORE= null, TMS_UPDATE= null  where test_roster_id = {testRosterId} and item_set_id = {itemSetId} ")
    void updateSissRecord(Integer testRosterId, Integer itemSetId) throws SQLException;
	...
	@JdbcControl.SQL(statement= "select ros.student_id as studentId, siss.item_set_id as itemSetId,subject,completion_status as completionStatus,test_completion_status as testCompletionStatus,siss.completion_date_time as completionDateTime,item_set_level as itemSetLevel,item_set_form as itemSetForm, ta.product_id as productId, ros.activation_status as rosterActivationStatus \n"  
	...
	+"and siss.completion_date_time is not null\n"
	+"and siss.completion_status='CO'"

	+"union \n"

	+"select ros.student_id as studentId, siss.item_set_id as itemSetId,subject,completion_status as completionStatus,test_completion_status as testCompletionStatus,siss.completion_date_time as completionDateTime,item_set_level as itemSetLevel,item_set_form as itemSetForm, ta.product_id as productId, ros.activation_status as rosterActivationStatus \n"  
	+",case ta.product_id when 4009 then 1 when 4010 then 1 when 4011 then 1 when 4012 then 1 else 2 end as ordr \n"
	+"from  student_item_set_status siss,  test_roster ros  , test_admin ta, item_set ii\n"
	+"where  siss.test_roster_id = ros.test_roster_id  \n"
	...
	+"and ii.sample='F'\n"
	+"and ros.student_id in ({sql: studentIds})\n"
	+"and ta.item_set_id= {testItemSetId}\n"
	//+"and ros.activation_status='AC'\n"
	+"and ta.activation_status='AC'\n"
	+"and ii.activation_status='AC'\n"
	...
	@JdbcControl.SQL(statement= "SELECT ros.student_id AS studentId,siss.item_set_id AS itemSetId,subject,siss.completion_status AS completionStatus, \n" 
		   ...
		   +"item_set_form AS itemSetForm,ta.product_id AS productId, ros.activation_status AS rosterActivationStatus \n"
	       +"FROM student_item_set_status siss,test_roster ros,test_admin ta,item_set ii \n"
	       +"WHERE siss.test_roster_id = ros.test_roster_id \n"
	       +"and ta.test_admin_id=ros.test_admin_id \n"
	       +"and ta.test_catalog_id = (select test_catalog_id from test_admin where test_admin_id = {testAdminId})  \n"
	       +"and siss.item_set_id=ii.item_set_id \n"
	       +"and ii.sample='F' \n"
		   +"and ii.item_set_level != 'L' \n"
		   +"and ros.student_id IN ({sql: studentIds}) \n" 
	...
	@JdbcControl.SQL(statement= "SELECT SUBJECT, TABE_LEVEL as TABELevel, TESTLET_FORM as TestletForm from TESTLET_FORMS_BY_SUBJECT_LEVEL \n" 
		+"WHERE subject = (SELECT distinct(subject) \n" 
		+"FROM item_set_ancestor isa JOIN item_set ii ON ii.item_set_id = isa.item_set_id \n" 
		+"WHERE sample='T' AND ii.item_set_type='TD' AND ancestor_item_set_id={itemSetId})", arrayMaxLength = 100000)	
	TestletLevelForm[] getTestletLevelFormsByItemSetId(Integer itemSetId) throws SQLException;
