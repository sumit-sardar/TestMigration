...
{ 
     ...
     *          
    */ 
    @JdbcControl.SQL(statement = "insert into student_item_set_status (  select distinct  test_roster_id, {itemSetId}, {completionStatus}, null, null,  {validationStatus}, null, null, null,  {timeExpired}, {order}, null, null, null, null, null,max(default_value) as default_value, {exemption}, {absent}  from (  select distinct  test_roster_id, cc.default_value  from  test_roster,  customer_configuration cc  where  cc.customer_id = {customerId}  and cc.customer_configuration_name = 'Roster_Status_Flag'  and test_admin_id in ({sql: adminList})  and student_id in ({sql: studentList})  union  select distinct  test_roster_id, null as default_value  from  test_roster  where  test_admin_id in ({sql: adminList})  and student_id in ({sql: studentList}))  group by test_roster_id)")
