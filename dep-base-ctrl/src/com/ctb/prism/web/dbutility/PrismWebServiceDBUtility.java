...
public class PrismWebServiceDBUtility {
	...
	private static final String SELECT_WS_ERROR_LOG = "SELECT *  FROM (SELECT WS_ERROR_LOG_KEY AS LOGKEY,               INVOKE_COUNT     INVKCOUNT,               STUDENT_ID       STDID,               ROSTER_ID        RSTRID,               SESSION_ID       SESSIONID,               WS_TYPE          WSTYP,               updated_date     UPDATEDATE          FROM WS_ERROR_LOG         WHERE STATUS = 'Progress'         ORDER BY UPDATED_DATE) TAB WHERE ROWNUM <= ?";
