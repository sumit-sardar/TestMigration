 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class CommandProcessorImportAssessmentFromContentQA implements CommandProcessor
{
     ...
     * 
     */
    public CommandProcessorImportAssessmentFromContentQA( Element rootElement_, Connection connection ) 
    {
        super();
    ...
    public Report processAssessment() 
    {
        ...
        try {
            ...
            AssessmentProcessorReport aAssessmentReport = ReportFactory.createAssessmentReport();
            ...
            try
            {
                stmt = this.session.connection().prepareCall( "{call CTPU_HANDLE_EXT_TST_IDN (?, ?)}");
	            stmt.setInt( 1, Integer.valueOf( ProductID ).intValue() );
	            stmt.setString( 2, id );
		        stmt.execute();
		        stmt.close(); 
		        aAssessmentReport.setSuccess( true );
	            List TSList = XMLUtils.getSchedulableUnitInAssessment( rootElement );
	            for ( int j = 0; j < TSList.size(); j++ )
	            {
		            SchedulableUnitReport TSReport = ReportFactory.createSchedulableUnitReport( false );
		            Element SchedulableUnitElement = ( Element )TSList.get( j );
		            String SchedulableUnitId = SchedulableUnitElement.getAttributeValue( "ID" );
		            TSReport.setId( SchedulableUnitId );
		            String title = SchedulableUnitElement.getAttributeValue( "Title" );
		            TSReport.setSubTestName( title );
		            String level =  SchedulableUnitElement.getAttributeValue( "Level" );
		            TSReport.setSubTestLevel( level );
		            List TDList = XMLUtils.getDeliverableUnitInSchedulableUnit( SchedulableUnitElement );
			        for ( int k = 0; k < TDList.size(); k++ )
			        {
			            DeliverableUnitReport TDReport = ReportFactory.createDeliverableUnitReport(true);
			            Element DeliverableUnitElement = ( Element )TDList.get( k );
			            String DeliverableUnitId = DeliverableUnitElement.getAttributeValue( "ID" );
			            TDReport.setId( DeliverableUnitId );
			            title = DeliverableUnitElement.getAttributeValue( "Title" );
			            TDReport.setSubTestName( title );
			            level =  DeliverableUnitElement.getAttributeValue( "Level" );
			            TDReport.setSubTestLevel( level );
			            String form = DeliverableUnitElement.getAttributeValue( "Form" );
			            TDReport.setSubTestForm( form );
			            List item_list = CommandProcessorUtility.getItemsUnderTD( DeliverableUnitElement );
            ...
            if (aAssessmentReport.isSuccess())
            {
                ...
                if (aAssessmentReport.isSuccess())
                {
	                ...
	                {
	                    aAssessmentReport.setSuccess( false );
	                    aAssessmentReport.setException( new Exception( "Perl script failed. Please contact Puru for details."));
	                }
                }
                else
    ...
    public void writePushInfo( String ext_tst_item_set_id, AssessmentProcessorReport aAssessmentReport )
    {
        String sql = "delete from INFA_PARAMETERS where JOB_NAME = 'ADS_ETL' and PARAM_NAME = 'EXT_TST_ITEM_SET_ID'";
        try
        {
            PreparedStatement pstatement = this.session.connection().prepareStatement( sql );
            pstatement.executeUpdate();
            pstatement.close();
            sql = "insert into INFA_PARAMETERS( JOB_NAME, PARAM_NAME, PARAM_VALUE ) "
                    	+ "values( 'ADS_ETL', 'EXT_TST_ITEM_SET_ID', '" + ext_tst_item_set_id + "')";
